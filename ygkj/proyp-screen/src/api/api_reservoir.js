...
// 大中型水库统计接口(可根据 水库风险研判类型 进行统计)
export function largeMediumReservoirsStatisticApi (adcd, riskType) {
  let itemCategory = 'scale'
  let itemValue = "-2"
  switch (riskType) {
    case 'processCnt':
  ...
  return axios.get("/mgt/bm/reservoirei/getStatisticBigScreen", {
    statistic: 'overCnt',
    adcd,
    itemCategory,
    itemValue
...
// 大中型水库落点接口(可根据 水库水位告警类型 及 水库风险研判类型 进行过滤)
export function largeMediumReservoirsListApi (adcd, warnType, riskType) {
  let itemCategory = 'scale'
  let itemValue = "-2"
  switch (warnType) {
    case 'DSFLZ':
      ...
      itemValue += ',超设计洪水位'
      break;
    case 'limitWaterLevel':
      itemCategory += ',limitWaterLevel'
      itemValue += ',超限制水位'
      break;
    default:
      break;
...
// 病险水库统计接口(可根据 水库风险研判类型 进行统计)
export function dangerousReservoirsStatisticApi (adcd, riskType) {
  let itemCategory = 'dangerousClass'
  let itemValue = "病险水库"
  switch (riskType) {
    case 'processCnt':
      itemCategory += ',processCnt'
...
// 病险水库落点接口(可根据 水库水位告警类型 及 水库风险研判类型 进行过滤)
export function dangerousReservoirsListApi (adcd, warnType, riskType) {
  let itemCategory = 'dangerousClass'
  let itemValue = "病险水库"
  switch (warnType) {
    case 'DSFLZ':
