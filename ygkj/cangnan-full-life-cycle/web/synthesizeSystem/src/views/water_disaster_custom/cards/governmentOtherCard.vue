 ...
 * @LastEditors: ysl
 * @LastEditTime: 2021-09-14 10:20:59
 * @Description: 其他报销
 * @FilePath: /synthesizeSystem/src/views/water_disaster_custom/cards/governmentOtherCard.vue
-->
<template>
  <div class="government-other-card">
    <panel-item
      title="其他报销"
      has-switch
      tc
      @tclick="tclick('其他报销')"
      :action-tabs="['总览', '报销次数', '报销金额']"
    ...
    >
      ...
      <template slot="title-switch">
        ...
        <i
          ...
          @click.stop="openDetailsDialog('其他报销')"
...
export default {
  ...
  mounted() {
		this.getOaOverView()
    this.otherExpenseCStatis()
    this.getOtherOverViewComponent()
  },
  methods: {
    ...mapActions(["SetDetailDialogParam", "SetotherReimbursementCount"]),
    // 设置C位数据
    async otherExpenseCStatis() {
      await oaApi.otherExpenseCStatis().then((res) => {
        // 获取c位数据
          this.SetotherReimbursementCount({
    ...
    async getOaOverView() {
      ...
      let str = this.activeTab == 1 ? "otherExpenseNumber" : "otherExpenseMoney"
    ...
    async getOtherOverViewComponent() {
      let opt = {
        month: Number(this.checkMonth.split(' ')[0]),
      };
      this.rankingList = []
      await oaApi.otherOverViewComponent(opt).then((res) => {
        let data = res.data
        this.totalCount = data.expenseTotalNum
    ...
    openDetailsDialog(val) {
			this.$emit("openDialog", "其他报销")
    },
    // 标题点击打开统计卡片
    tclick(title) {
      this.$emit("tclick", "其他报销");
    },
    handleTabChange(val) {
      this.activeTab = val;
			this.barOption.setting.yAxisName[0] = this.activeTab == 1 ? "次" : "万元"
      this.activeTab == 0 ? this.getOtherOverViewComponent() : this.getOaOverView();
    },
    barConfig(options) {
...
<style lang="scss">
.government-other-card .panel-item .panel-item__content--tabs .tab-button,
.government-other-card .panel-item .panel-item__title--tabs .tab-button {
  display: inline-block;
  font-size: 16px;
  padding: 0 12px;
}
.government-other-card .panel-item .panel-item__content--tabs .tab-button.active,
.government-other-card .panel-item .panel-item__title--tabs .tab-button.active {
  padding: 0 12px;
}
</style>
