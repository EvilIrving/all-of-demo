  ...
  <div class="government-assets-card">
    <panel-item title="水厂供水" tc @tclick="tclick">
      <div class="charts-panel">
        <!-- 水厂 -->
        <sub-title-click-card title="水厂" type="radio" :radioList="['数量', '人口']" @radioChange="areaChange">
          <div slot="title-operate">
            ...
            />
            <i title="查看详表" class="icon-liebiao icon-sty" @click.stop="openDetailsDialog('水厂')"/>
          </div>
          <div class="chart-panel">
              ...
              <div class="legend-panel">
                <div class="legend-item" v-for="(item, index) in pieChartData" :key="index">
                  <div>
                    ...
                    <span class="text" :style="{ width: areaType == 0 ? '100px' : '60px' }">
                      {{ item.name }}
                    </span>
                  </div>
                  <div>
            ...
            <!-- 水厂行政区划统计图 -->
            <div class="line-box">
              <span style="margin-left:30px;">{{ areaTabType == "数量" ? "个" : "万人" }}</span>
              <div id="stackedColumn" style="width:100%;height:100%;" />
            </div>
          </div>
        </sub-title-click-card>
        <!-- 水厂运行状态 -->
        <sub-title-click-card title="水厂运行状态" type="radio" :radioList="['数量', '人口']" @radioChange="warnChange">
          <div slot="title-operate">
            <i
              title="地图落点"
              :class="[
          ...
          <div class="chart-panel">
            <div class="count-box">
              <div class="all-count">
                总数 <span>{{ statisticsData.total.num == null ? '-' : statisticsData.total.num }}</span>
                {{ warnTabType == "数量" ? "个" : "万人" }}
              </div>
              <div class="count4-box">
                <div class="count4-box-item green">
                  <p> 正常供水 </p>
                  <p class="num"> {{ warnTabType == "数量" ? statisticsData.norm.num : statisticsData.norm.pop }}</p>
                </div>
                <div class="count4-box-item yellow">
                  <p> 供水预警</p>
                  <p class="num">{{ warnTabType == "数量" ? statisticsData.volQuaWarn.num : statisticsData.volQuaWarn.pop }}</p>
                </div>
                <div class="count4-box-item orange">
                  <p>水质预警</p>
                  <p class="num">{{ warnTabType == "数量" ? statisticsData.volumeWarn.num : statisticsData.volumeWarn.pop }}</p>
                </div>
                <div class="count4-box-item red">
                  <p>水质供水预警</p>
                  <p class="num">{{ warnTabType == "数量" ? statisticsData.qualityWarn.num : statisticsData.qualityWarn.pop }}</p>
                </div>
              </div>
            </div>
            <!-- 水厂运行状态行政区划统计 -->
            <div class="line-box">
              <span style="margin-left:30px;">{{ warnTabType == "数量" ? "个" : "万人" }}</span>
              <div id="runningStatus" style="width:100%;height:100%;" />
            </div>
          </div>
        </sub-title-click-card>
      </div>
  ...
  methods: {
      ...
      await waterResourceApi.waterWorksStats(opt).then((res) => {
        res.data.forEach((item) => {
          this.pieChartData.push({
            name: this.areaTabType == "数量" && this.areaType == 0 ? item.wasuRang : this.areaTabType == "数量" && this.areaType == 1 ? item.engType : this.areaTabType == "人口" && this.areaType == 0 ? item.wasuRang : item.engType,
            value: this.areaTabType == "数量" && this.areaType == 0 ? item.num : this.areaTabType == "数量" && this.areaType == 1 ? item.num : this.areaTabType == "人口" && this.areaType == 0 ? item.pop : item.pop,
          });
        });
    ...
    drawPointArea(btn) {
      ...
      let style1 = {
        ...
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/city-icon.png"),
        src: require("@/assets/images/city-icon-aggregation.png"),
      };
      let style2 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/town-icon.png"),
        src: require("@/assets/images/town-icon-aggregation.png"),
      };
      let style3 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/union-icon.png"),
        src: require("@/assets/images/union-icon-aggregation.png"),
      };
      let style4 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/single-icon.png"),
        src: require("@/assets/images/single-icon-aggregation.png"),
      };
      let pointList1 = [];
      let pointList2 = [];
      let pointList3 = [];
    ...
    drawPointWarn(btn) {
      ...
      let style1 = {
        ...
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/norm-icon.png"),
        src: require("@/assets/images/norm-icon-aggregation.png"),
      };
      let style2 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/volumeWarn-icon.png"),
        src: require("@/assets/images/volumeWarn-icon-aggregation.png"),
      };
      let style3 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/qualityWarn-icon.png"),
        src: require("@/assets/images/qualityWarn-icon-aggregation.png"),
      };
      let style4 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        srcs: require("@/assets/images/volQuaWarn-icon.png"),
        src: require("@/assets/images/volQuaWarn-icon-aggregation.png"),
      };
      let pointList1 = [];
      let pointList2 = [];
      let pointList3 = [];
