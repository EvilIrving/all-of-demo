  ...
  <div class="current-typhoon">
    <div class="simple-info">
      <div class="simple-info__title">
        ...
        >
        <span v-if="TyphoonPointData.power" class="typhoon-level">{{ TyphoonPointData.power}}</span>
      </div>
       <div class="simple-info__effect">
        <span>预计登陆地点：{{TyphoonPointData.land ? eval(TyphoonPointData.land).join(","):'-'}}</span>
    ...
    <el-tabs v-model="activeTab" type="card" stretch class="current-typhoon">
      ...
      <el-tab-pane name="effect" label="台风影响">
        <TyphoonEffect :point-data="TyphoonPointData.point" :typhoonName="TyphoonPointData.typhoonInfo.name" />
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
