    ...
    >
      ...
      <template slot="title-switch">
        ...
        <i
          ...
          @click.stop="openDetailsDialog('水资源量（公报）')"
...
export default {
  ...
  methods: {
    ...
    showAreaPolygon() {
      ...
      let arr = this.barData1.rows.map(item => { return item.totalWra });
      if (this.activeTab == 1) {
        rows = this.barData2.rows;
        arr = this.barData2.rows.map(item => { return item.avgWrUse });
      }
      ...
      rows.map(item => {
        ...
        item.attr = item;
        if (this.activeTab == 0) {
          item.style = this.getColor(item.totalWra, max, min);
        } else {
          item.style = this.getColor(item.avgWrUse, max, min);
        }
      });
