    ...
    <div class="collapse-panel__content">
      ...
      <panel-item title="水库" hasSwitch>
        <template slot="title-switch">
          <i title="地图落点" @click="handleReservoirChange('skbtn')"
              :class="[
        ...
        <div class="reservoir-sty">
          <div>
            <p><i class="icon-chao-jing icon-sty" />超警 <span> {{rsvrReplayStatsData != null ? rsvrReplayStatsData.warnNum : '-'}} </span> 座</p>
            <div>
              <p>超警时间最长 </p>
              <p>
                <span> {{rsvrReplayStatsData.maxWarnTimeRsvr != null ? rsvrReplayStatsData.maxWarnTimeRsvr.resName : '-'}} </span>
                <span>{{rsvrReplayStatsData.maxWarnTimeRsvr != null ? rsvrReplayStatsData.maxWarnTimeRsvr.overWarnMinutes : '-'}}</span>min
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="reservoir-sty">
          <div style="background:none;padding:0;" class="cut-off-rule">
            <p style="padding-bottom:0;border-bottom:none;">
              ...
              <span style="color: #1CFFF5;"> {{rsvrReplayStatsData != null ? rsvrReplayStatsData.totalStorage : '-'}} </span> 万m³
            </p>
            <div style="padding-top: 5px;">
              <p>拦截蓄水量最大</p>
              <p>
                <span style="color: #1CFFF5;"> {{rsvrReplayStatsData.maxStorageRsvr != null ? rsvrReplayStatsData.maxStorageRsvr.resName : '-'}}</span>
                <span style="color: #1CFFF5;"> {{rsvrReplayStatsData.maxStorageRsvr != null ? rsvrReplayStatsData.maxStorageRsvr.capacitySub : '-'}}</span> 万m³
              </p>
            </div>
          </div>
        </div>
        <div class="reservoir-sty">
          <div style="background:none;padding: 0;">
            <p style="padding-bottom:0;border-bottom:none;">
              ...
              <span style="color: #1CFFF5;">  {{rsvrReplayStatsData != null ? rsvrReplayStatsData.totalDrainage : '-'}} </span>
              <span style="font-size: 18px;color: #FFF;">万m³</span>
              <span style="color: #1CFFF5; margin-left:10px;"> {{rsvrReplayStatsData != null ? rsvrReplayStatsData.drainageNum : '-'}} </span>座
            </p>
            <div style="padding-top: 5px;">
              <p>泄洪量最大</p>
              <p>
                <span style="color: #1CFFF5;"> {{ rsvrReplayStatsData.maxDrainageRsvr != null ? rsvrReplayStatsData.maxDrainageRsvr.resName : '-'}} </span>
                <span style="color: #1CFFF5;"> {{ rsvrReplayStatsData.maxDrainageRsvr != null ? rsvrReplayStatsData.maxDrainageRsvr.drainage : '-'}}</span> 万m³
              </p>
            </div>
          </div>
        </div>
      ...
      <panel-item title="水闸" hasSwitch>
        <template slot="title-switch">
          <i title="地图落点" @click="handleSluiceChange('szbtn')" :class="[
        ...
        <div class="reservoir-sty">
          <div style="background:none;" class="cut-off-rule">
            <p style="padding-bottom:0;border-bottom:none;" class="cut-off-rule">
              ...
              <span style="color: #1CFFF5;"> {{wrpWagaReplayStatsData != null ? wrpWagaReplayStatsData.drainageTotal : '-'}} </span> 万m³
            </p>
          </div>
        </div>
        <div class="reservoir-sty">
          <div style="background:none;padding: 0;">
            <p style="padding-bottom:0;border-bottom:none;">
              ...
              <span style="color: #1CFFF5;"> {{wrpWagaReplayStatsData != null ? wrpWagaReplayStatsData.openGateNum : '-'}} </span> 个
            </p>
            <div style="padding-top: 5px;">
              <p>开闸时间最长</p>
              <p>
                <span style="color: #1CFFF5;"> {{wrpWagaReplayStatsData.maxOpenGateTimeWaga == null ? '-': wrpWagaReplayStatsData.maxOpenGateTimeWaga.wagaName }} </span>
                <span style="color: #1CFFF5;"> {{wrpWagaReplayStatsData.maxOpenGateTimeWaga != null ? wrpWagaReplayStatsData.maxOpenGateTimeWaga.openGateMinutes : '-'}} </span> min
              </p>
            </div>
          </div>
        </div>
      ...
      <panel-item title="河网" hasSwitch >
        <template slot="title-switch">
          <i title="地图落点" @click="handleRiverNet('hwbtn')" :class="[
        ...
        <div class="reservoir-sty">
          <div>
            <p><i class="icon-chao-jing icon-sty" />超警 <span> {{rivernetReplayStatsData.overWarnNum != null ? rivernetReplayStatsData.overWarnNum : '-'}} </span> 座</p>
            <div>
              <p>超警时间最长</p>
              <p>
                <span> {{rivernetReplayStatsData.maxOverWarnTime != null ? rivernetReplayStatsData.maxOverWarnTime.rnName : '-'}} </span>
                <span> {{rivernetReplayStatsData.maxOverWarnTime != null ? rivernetReplayStatsData.maxOverWarnTime.overWarnMinutes : '-'}} </span> min
              </p>
            </div>
          </div>
        </div>
