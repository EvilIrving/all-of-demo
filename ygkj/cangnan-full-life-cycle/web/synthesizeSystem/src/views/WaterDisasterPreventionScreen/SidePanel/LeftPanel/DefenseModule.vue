        ...
        <panel-item :inSimulation="previewModule" title="实时降雨" hasSwitch>
          ...
          <template slot="title-switch">
            ...
            <i
              title="模拟演练"
              style="height: 18px"
              :class="['icon-moniyanlian', 'icon-sty']"
              @click="doDrill()"
            >
            </i>
            <i
  ...
  computed: {
    ...
    realtimeMaxPoint() {
      ...
      return {
        ...
        value:
          Number(this.realtimePreMaxList.maxPointRainfall || 0).toFixed(1) || 0,
        type: "station",
        lat: lat,
        lng: lng,
      };
    ...
    realtimeMaxArea() {
      return {
        ...
        value:
          Number(this.realtimePreMaxList.areaMaxRainfall || 0).toFixed(1) || 0,
        type: "area",
      };
    },
    realtimeMaxBasin() {
      return {
        ...
        value:
          Number(this.realtimePreMaxList.basinMaxRainfall || 0).toFixed(1) || 0,
        type: "basin",
      };
    },
    realtimeMaxList() {
  ...
  methods: {
    ...
    getPreviewRainfallData(areaName) {
      ...
      let opt = {
        areaName: "",
        drill: true,
        endTime: new Date(this.formatterCurrentTime()).format(
          "yyyy-MM-dd hh:mm:ss"
        ),
      };
      if (areaName && areaName != "温州市") {
      ...
      disasterPreventionApi.realtimeRainfallByAreaName(opt).then((res) => {
        ...
        this.rainList =
          this.realtimePreList[mapArray[this.realtimePreCardSelect]];
      });
      if (!areaName) {
    ...
    doPreview() {
      if (this.typhoonVal) {
        ...
        let startTime = this.typhoonData.beginTime
          ? new Date(this.typhoonData.beginTime).getTime()
          : "";
        let endTime = this.typhoonData.endTime
          ? new Date(this.typhoonData.endTime).getTime()
          : "";
        this.setPreviewData(this.typhoonData);
        this.setStartTimeatmap(startTime);
        this.setEndTimeatamp(endTime);
        let preStartTime = this.typhoonData.futureBegin
          ? new Date(this.typhoonData.futureBegin).getTime()
          : "";
        let preEndTime = this.typhoonData.futureEnd
          ? new Date(this.typhoonData.futureEnd).getTime()
          : "";
