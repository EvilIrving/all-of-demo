    ...
    <div class="collapse-panel__content">
      ...
      <panel-item title="潮位" hasSwitch>
        <template slot="title-switch">
          <i title="地图落点" @click="handleTideChange('cwbtn')"
              :class="[
        ...
        <div style="margin:10px 0;">
          <capsule-tabs v-model="tideStaName" :tabs="['温州', '龙湾', '瑞安', '鳌江']" @change="tideStaNameTabChange" />
        </div>
        <!-- 潮位图表 -->
      ...
      <panel-item title="水毁" hasSwitch>
        ...
        <div style="margin-bottom:10px;">
          <capsule-tabs v-model="realtimePreChart" :tabs="['行政区划', '工程类型']" @change="forecastChartTabChange" />
        </div>
        <!-- 水毁图表 -->
        <ve-histogram
          height="195px"
      ...
      <panel-item title="水利设施保险" hasSwitch >
        <template slot="title-switch">
          <i title="地图落点" @click="handleInsuranceChange('bxbtn')" :class="[ hwbtn % 2 == 0 ? 'icon-yinzhang-active' : 'icon-yinzhang-1', 'icon-sty' ]" />
        ...
        <div>
          <el-row>
            <el-col :span="12" class="insurance-item">
              <div @click="insuranceChange('理赔数量')" :class="['claim-settlement-sty',realtimePreChartTab == '理赔数量' ? 'active':'']">
            ...
            <el-col :span="12" class="insurance-item">
              <div @click="insuranceChange('理赔金额')" :class="['claim-settlement-sty',realtimePreChartTab == '理赔金额' ? 'active' : '']">
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      let eTime = this.$moment().subtract(0, "days").format("YYYY-MM-DD HH") + ":00:00";
  ...
  methods: {
    ...
    handleEngineeringChange(val) {
      ...
      this.realtimeChartOption.setting.yAxisName = [
        this.riverNetworkType == 0 ? "mm" : "万m³",
      ];
...
<style lang="scss" scoped>
  ...
  .collapse-panel__content {
    ...
    .claim-settlement-sty {
      ...
      justify-content: center;
      &.active{
        background: linear-gradient(to bottom, #1cfff485, #1cfff485) left top no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) left top no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) right top no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) right top no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) left bottom no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) left bottom no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) right bottom no-repeat, linear-gradient(to bottom, #1cfff485, #1cfff485) right bottom no-repeat;
