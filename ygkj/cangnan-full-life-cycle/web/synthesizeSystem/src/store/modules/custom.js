...
const screen = {
	state: {
		...
		isCollapse: false,
		isBigScreen: true,
		componentLoading: true, //场景切换loading
	...
	actions: {
		...
		GetSceneList({ commit }, data) {
			return new Promise(resolve => {
				disasterPreventionApi.listScene(data).then(res => {
					if (res.code == 0) {
						commit('SET_SCENE_LIST', res.data);
						// let defaultScene = {
						// 	name:res.data[0].sceneName,
						// 	id:res.data[0].id,
						// };
			...
			return new Promise(resolve => {
				disasterPreventionApi.listFullBigScreen(data).then(res => {
					...
					if (res.code == 0) {
						...
						// console.log(tempArr,88888)
						if (rootGetters.warningListShow && getters.BsName == '水灾害防御') {
							tempArr.unshift('报警');
						}
						resolve(tempArr);
						//设置选中卡片组件
