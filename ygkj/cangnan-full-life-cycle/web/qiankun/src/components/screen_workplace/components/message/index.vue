<template>
  <div class="main-workplace-message">
    <div class="workplace-main__left">
      ...
      <div class="role-info">
        <div class="dept-roles" v-for="(item, index) in roleArr" :key="index">
          <p class="department">{{ item.type }}</p>
          <el-row class="role-list" :gutter="20">
            <el-col :span="8" v-for="(child, idx) in item.roles" :key="idx">
              <div class="role-item" :title="child.NAME">{{ child.NAME }}</div>
            </el-col>
          </el-row>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getPerRoles() {
      messageApi.getPerRoles().then(res => {
        if (res.code == 0) {
