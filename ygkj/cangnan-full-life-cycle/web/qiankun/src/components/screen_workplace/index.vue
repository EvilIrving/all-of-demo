<template>
  ...
  >
    ...
    <div class="workplace-main">
      ...
      <DataSheet v-if="activeIndex == 1" />
      <Collect v-if="activeIndex == 2" />
      <MyWork v-if="activeIndex == 3" />
      <Track v-if="activeIndex == 4"/>
    </div>
  </el-dialog>
</template>
<script>
...
import MyWork from "./components/work/index.vue";
import Track from "./components/track/index.vue";
export default {
  name: "Workplace",
  props: { visiable: Boolean },
  components: { Message, Collect, DataSheet,MyWork,Track },
  data() {
    return {
      activeIndex: 0,
      nav: [
        { title: "我的消息", icon: "icon-workplace-msg" },
        { title: "我的数据", icon: "icon-workplace-data" },
        { title: "我的收藏", icon: "icon-workplace-fav" },
        { title: "我的工作", icon: "icon-workplace-work" },
        { title: "我的轨迹", icon: "icon-workplace-info" },
      ],
    };
  },
  methods: {
