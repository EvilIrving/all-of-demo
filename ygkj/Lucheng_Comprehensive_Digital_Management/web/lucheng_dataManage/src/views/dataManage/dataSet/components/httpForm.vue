...
<template>
  <div class="form-content">
    <el-form ref="form" :model="form" :rules="rules" label-width="96px">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="请求体类型" prop="contentType">
            <el-select
        ...
        <el-col :span="12">
          <el-form-item label="请求">
            <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
              <el-tab-pane label="请求体JSON" name="body">
                <SqlEditor
              ...
              <el-tab-pane label="请求头JSON" name="header"
                ><SqlEditor
                  ref="requestHeaderEditor"
                  :value="requestHeader"
                  :editorHeight="360"
                ></SqlEditor
              ></el-tab-pane>
      ...
      <el-row>
        <el-col :span="12">
          ...
          >
            <el-input
              v-model="form.responsePath"
              placeholder="请输入响应JSON数据路径"
            />
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  components: { SqlEditor, TablePage },
  data() {
    return {
      form: {
        dataSetName: "",
      ...
      rules: {
        ...
        requestUrl: [this.$ruler("请求地址")],
        requestMethod: [this.$ruler("请求方式")],
      },
      id: "",
      setDetailId: "",
      oldRequestUrl: "",
      oldRequestMethod: "GET",
      oldContentType: 1,
      oldResponsePath: "",
  ...
  methods: {
    ...
    reset() {
      ...
      this.oldRequestUrl = "";
      this.oldRequestMethod = "GET";
      this.oldContentType = 1;
      this.oldResponsePath = "";
      this.oldRequestBody = "";
      this.requestBody = "";
      this.oldRequestHeader = "";
      this.requestHeader = "";
      ...
      this.$refs.requestBodyEditor.setTextValue("");
      this.$refs.requestHeaderEditor.setTextValue("");
    },
      ...
      getDataSetById(this.setId).then((res) => {
        if (res.data) {
          ...
          if (res.data.setDetail) {
            ...
            this.form.requestUrl = res.data.setDetail.requestUrl;
            this.form.requestMethod = res.data.setDetail.requestMethod;
            this.form.contentType = res.data.setDetail.contentType;
            this.form.responsePath = res.data.setDetail.responsePath;

            this.oldRequestUrl = res.data.setDetail.requestUrl;
            this.oldRequestMethod = res.data.setDetail.requestMethod;
            this.oldContentType = res.data.setDetail.contentType;
            this.oldResponsePath = res.data.setDetail.responsePath;
            this.oldRequestBody = res.data.setDetail.responsePath;
            this.requestBody = res.data.setDetail.requestBody;
            this.oldRequestHeader = res.data.setDetail.requestHeader;
            this.requestHeader = res.data.setDetail.requestHeader;
          }
    ...
    handleClick() {
      // if (this.activeName == "header" && this.refreshFlag) {
      //   this.$refs.requestHeaderEditor.refreshs();
    ...
    // 获取预览数据
    getPreviewData() {
      this.requestBody = this.$refs.requestBodyEditor.getTextValue();
      this.requestHeader = this.$refs.requestHeaderEditor.getTextValue();
    ...
    runPreviewData() {
      ...
      if (this.form.requestUrl !== this.oldRequestUrl) {
        fieldUpdate = true;
      }
      if (this.form.requestMethod !== this.oldRequestMethod) {
        fieldUpdate = true;
      }
      if (this.form.contentType !== this.oldContentType) {
        fieldUpdate = true;
      }
      if (this.oldRequestBody !== this.requestBody) {
        fieldUpdate = true;
      }
      if (this.oldRequestHeader !== this.requestHeader) {
        fieldUpdate = true;
      }
      if (this.form.responsePath !== this.oldResponsePath) {
        fieldUpdate = true;
      }

      this.oldRequestUrl = this.form.requestUrl;
      this.oldRequestMethod = this.form.requestMethod;
      this.oldContentType = this.form.contentType;
      this.oldRequestBody = this.requestBody;
      this.oldRequestHeader = this.requestHeader;
      this.oldResponsePath = this.form.responsePath;
      ...
      let setDetail = {
        ...
        responsePath: this.form.responsePath,
      };
      let paramList = [];
      let param = {};
      let fieldList = [];
        ...
        if (res.data) {
          ...
          if (res.data.res) {
            if (Array.isArray(res.data.res)) {
              ...
              this.tableData.push(res.data.res);
            }
          }
    ...
    // 格式化对象
    formatObj(row, column, cellValue) {
