...
let CustomIframe = {
	...
	render() {
		return <iframe style={{ width: "100%", height: "100%", 'min-height': '450px' }} frameborder="0" src={this.src} />;
	},
};
/** 工程信息 用了俩组件 简单二次封装下*/
let ProjectInfo = {
...
export default {
	...
	render() {
		let title = this.pointData.name || this.pointData.hystName || this.pointData.cwsName || this.pointData.gateName || this.pointData.wagaName || this.pointData.pustName;
		...
		return (
			<PrimaryDialog
				...
				main-title={title}
				onAnalysis={() => this.$emit('analysis')} has-analysis={this.analysisFlag} width="1100px"
				onHandleClose={() => this.handleClose()} fullscreenFlag={true}
	...
	methods: {
		...
		renderContent() {
			...
			if (this.displayTabList[this.currentTab] == "运行管理") {
				this.frameUrl = `https://sk.zjwater.com/cp/skip/reservoirDetailSingle?prcd=${this.pointData.code}&name=${encodeURI(encodeURI(title))}&fromMap=1&pName=&cName=&token=${this.token}`;
			}
