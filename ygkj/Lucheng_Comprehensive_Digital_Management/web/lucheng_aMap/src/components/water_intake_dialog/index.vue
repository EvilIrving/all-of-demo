<!--
 * @Author: ly
 * @Date: 2021-08-18 09:47:17
 * @LastEditTime: 2021-08-19 18:12:26
...
<script>
  ...
  export default {
    ...
    created() {},
    mounted() {
      this.wiuCode = this.WaterIntakeData.wiuCode;
      this.listUseByMonth();
    ...
    methods: {
      ...
      async listUseByMonth() {
        ...
        await hydraulicApi.listUseByMonth(opt).then((res) => {
          this.tbList1 = res.data;
          res.data.forEach((item) => {
            this.chartData.name.push(item.year + '-' + item.month);
      ...
      async currentWptOfWiu() {
        ...
        await hydraulicApi.currentWptOfWiu(opt).then((res) => {
      ...
      async wiuDocList4Config() {
        ...
        await hydraulicApi.wiuDocList4Config(opt).then((res) => {
      ...
      setBaseInfoList() {
        ...
        this.listData = this.WaterIntakeData;
      },
      changeActiveTab(val) {
        this.activeTab = val;
        switch (val) {
      ...
      generateDataZoom(count, length, setting = { axix: "x", index: [0] }) {
        let ret = {
          type: "slider",
          start: 0, //默认为0
      ...
      chartOpt() {
        ...
        let options = {
          dataZoom: _this.generateDataZoom(6, this.chartData.name.length),
          grid: {
            top: "10%",
            bottom: "5%",
            left: "0%",
