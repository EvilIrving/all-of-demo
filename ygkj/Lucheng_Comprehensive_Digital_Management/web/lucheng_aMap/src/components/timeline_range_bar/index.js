...
export default {
	...
	methods: {
		...
		handlePrevButton() {
			if (this.position >= 1) {
				this.position--;
				this.handleSliderChange(this.vallue);
			}
		},
		handleNextButton() {
			if (this.isBefore) {
		...
		async getIsohyetArea(opt, type) {
			...
			if (type) {
				areaUrl = "http://112.17.127.75:8008/arcgis/rest/services/gps/dengzhiPIDWReal0605/GPServer/dengzhiPIDWReal0605/submitJob";
			}
			await axios({
				url: areaUrl,
				method: "post",
				...
				let interVal = setInterval(async () => {
					...
					if (type) {
						url = "http://112.17.127.75:8008/arcgis/rest/services/gps/dengzhiPIDWReal0605/GPServer/dengzhiPIDWReal0605/jobs/" + res.data.jobId + "/results/Idwres?f=pjson";
					}
					await axios({
						url: url,
						method: "get",
