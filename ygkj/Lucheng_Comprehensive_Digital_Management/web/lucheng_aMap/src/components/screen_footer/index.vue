...
import FooterDate from "./components/FooterDate.vue";
import SmartCompare from "./components/SmartCompare.vue";
...
export default {
  ...
  components: { FooterActionBar, FooterInfo, FooterDate, SmartCompare },
  mounted() {},
  methods: {
      ...
      queryMapserver("/温州流域/MapServer/0/query", {
        ...
        if (res.features && res.features.length > 0) {
          esri2geo(res, (err, data) => {
            ...
            this.mapBind
              .getView()
              .fit(this.basinLayer.getSource().getFeatures()[0].getGeometry(), {
    ...
    mapTypeChange(value) {
      ...
      let wenzhouVecLinght = new TileLayer({
        ...
        zIndex: 0,
        source: new XYZ({
          url: "http://112.17.127.75:8008/arcgis/rest/services/wenzhou/wenzhou_bsend/MapServer/tile/{z}/{y}/{x}",
          projection: "EPSG:4326",
        }),
      });
      //温州水行政区划矢量-黑色
      let wenzhouVecDark = new TileLayer({
        ...
        zIndex: 0,
        source: new XYZ({
          url: "http://112.17.127.75:8008/arcgis/rest/services/wenzhou/wenzhou_fangxunend/MapServer/tile/{z}/{y}/{x}",
          projection: "EPSG:4326",
        }),
      });
      //温州水行政区划矢量-白色
      let wenzhouVecWhite = new TileLayer({
        ...
        zIndex: 0,
        source: new XYZ({
          url: "http://112.17.127.75:8008/arcgis/rest/services//wenzhou/wenzhou_bsWhite/MapServer/tile/{z}/{y}/{x}",
          projection: "EPSG:4326",
        }),
      });
      //浙江水利底图-蒙层
      this.zhejiangSLMCLayer = new TileLayer({
        ...
        zIndex: 2,
        source: new XYZ({
          url: "http://112.17.127.75:8008/arcgis/rest/services/wenzhou/shuiliditu/MapServer/tile/{z}/{y}/{x}",
          projection: "EPSG:4326",
        }),
      });
      //浙江水利底图-蒙层黑色
      this.zhejiangSLMCHSLayer = new TileLayer({
        ...
        zIndex: 2,
        source: new XYZ({
          url: "http://112.17.127.75:8008/arcgis/rest/services/wenzhou/wzmchdnew/MapServer/tile/{z}/{y}/{x}",
          projection: "EPSG:4326",
        }),
      });
      //地图填充图层
      ...
      if (value == "wnzhouImgLayer") {
        this.changeTimelinebarShow(true);
        this.mapBind.addLayer(this.zhejiangSLMCHSLayer);
      } else {
        this.mapBind.removeLayer(this.zhejiangSLMCHSLayer);
        ...
        } else if (boderListnew.includes(value)) {
          this.mapBind.addLayer(this.zhejiangSLMCHSLayer);
        }
        // if (value == "zhejiangSLLayer") {
        //   this.mapBind.addLayer(this.wenzhouTCLayer);
        // }
        const list = [
          "zhejiangmainSLMCLayer",
          "zhejiangmainSLLayer",
  ...
  computed: {
    ...mapGetters(["mapChangeModule","MapComponent"]),
  },
};
</script>
...
.map-footer {
  ...
  ::v-deep .compare-detail-box{
    ...
    background-image: linear-gradient(
      180deg, rgba(53, 149, 227, 0.4) 0%, rgba(25, 59, 197, 0.4) 100%);
