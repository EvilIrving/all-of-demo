 ...
 * @Descripttion:
 * @FilePath: /lucheng_aMap/src/utils/mapUtil.js
 */
...
export function drawPoint(coordinates, styleObj) {
	...
	let vectorLayer = new Vector({
		...
		style: (feature) => {
			...
			if (mapZoom > 11) {
				const titleData = featureData.text ? featureData.text : '-';
				titleText = new Text({
...
export function drawSkRainPoint(coordinates, type) {
	...
	let vectorLayer = new Vector({
		...
		style: (feature) => {
			...
			let style;
			let titleText = null;
			const featureData = feature.getProperties().featureProperties.data;
			const imgSrc = featureData.src
			if (mapZoom > 11) {
				let data = featureData.drp
				if (featureData.drp && Number(featureData.drp) < 0) {
					data = '-'
				}
				const titleData = `${featureData.stationName || '-'} 降雨量:${data}mm`;
				titleText = new Text({
					text: titleData, // 路线标签文字
					font: "normal 14px 微软雅黑", //字体样式
					fill: new Fill({
			...
			case '防汛仓库':
				{
					latitude = coordinates[i].hystLat || coordinates[i].latitude;
					longitude = coordinates[i].hystLong || coordinates[i].longitude;
					imgSrc = require('@/assets/images/legend-warehouse.png');
				}
				break;
			case '救援队伍':
				{
					latitude = coordinates[i].hystLat || coordinates[i].latitude;
					longitude = coordinates[i].hystLong || coordinates[i].longitude;
					imgSrc = require('@/assets/images/legend-ranks.png');
				}
				break;
			case '避灾点':
				{
					...
					imgSrc = require('@/assets/images/map-bizaidian.png');
					scale = 0.5
				}
				break;
			default: {
