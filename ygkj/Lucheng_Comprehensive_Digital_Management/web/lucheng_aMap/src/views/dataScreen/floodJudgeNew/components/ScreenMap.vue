    ...
    <div id="popup3" class="ol-popup-project">
      ...
      <div v-if="popupData.stName == '桥墩坝上'">
        <p class="font-orange">
          <i class="icon-warn-tips"></i>
          当前位置发生预警， 前往
          <span class="go-btn" @click="bigScreenGo('/jointDispatch')"
            >调度</span
          >
          前往
...
export default {
  ...
  methods: {
    ...
    cleanAllLayer() {
      ...
      this.map.removeLayer(this.stagnantLayer);
      this.map.removeLayer(this.riskLayer);
      this.map.removeLayer(this.imgLayer);
      // clearInterval(this.floodAnalysisTimer);
    },
    //等雨面量
    ...
    async _drawStagnantPoint(data) {
      if (this.stagnantLayer != null) {
        this.map.removeLayer(this.stagnantLayer);
        this.stagnantLayer = null;
      }
      this.stagnantLayer = await drawStagnantPoint(data);
      this.map.addLayer(this.stagnantLayer);
      this.lodeFloodAnalysisLayer();
    },
    _drawRiskPoint(data) {
      if (this.riskLayer != null) {
        this.map.removeLayer(this.riskLayer);
        this.riskLayer = null;
      }
      this.riskLayer = drawRiskPoint(data);
      this.map.addLayer(this.riskLayer);
    },
    lodeFloodAnalysisLayer() {
      ...
      //
      // let flagPlay = 1
      // this.floodAnalysisTimer = setInterval(() => {
      //   flagPlay == 7 ? flagPlay = 1 : flagPlay = flagPlay;
      //   let imgSource = new ImageStatic({
      //     url: require("@/assets/images/pattern-spot/0" + flagPlay + ".png"),//这里添加静态图片的地址
      //     projection: "EPSG:4326",
      //     imageExtent: [
      //       119.927915940436,
      //       27.4949876579581,
      ...
      //     ], //WGS-84 经纬度数据范围
      //   });
      //   this.imgLayer.setSource(imgSource);
      //   flagPlay++;
      // }, 1000)

      let imgSource = new ImageStatic({
        url: require("@/assets/images/pattern-spot/06.png"), //这里添加静态图片的地址
        projection: "EPSG:4326",
        imageExtent: [
          120.425858358571,
          27.76366265557471,
