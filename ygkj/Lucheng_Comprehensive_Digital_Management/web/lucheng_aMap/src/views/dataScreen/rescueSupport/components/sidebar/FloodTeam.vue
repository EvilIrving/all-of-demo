<!--
 * @Author: hanyu dyn
 * @LastEditTime: 2021-01-27 11:31:40
 * @Description: 防汛队伍右边栏
 * @FilePath: /FloodDisasterDispatch/src/views/dataScreen/rescueSupport/components/sidebar/FloodTeam.vue
-->
<template>
...
export default {
  ...
  mounted() {
    if(this.tableData.length === 0){
      ...
      this.$parent.$refs.map.showPoint("team", this.tableData);
    }
  },
  methods: {
    //列表
    getList(){
      let that = this
      this.$store.dispatch('getFloodTeamList',{
        params: {
          areaCode: this.areaCode,
          name: this.name
        },
        callback: (list) => {
          that.$parent.$refs.map.showPoint("team", list);
        }
      })
    }
  },
