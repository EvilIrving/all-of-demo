	...
	<div class="project-side" ref="stationSide">
		...
		<div class="statistics-chart" ref="statisticsChart">
			<div class="chart-tabs">
				<span :class="[tabType == '条件选择' ? 'active' : '']" @click="changeTabType('条件选择')">条件选择</span>
				<span :class="[tabType == '区域' ? 'active' : '']" @click="changeTabType('区域')">区域</span>
				<span :class="[tabType == '规模' ? 'active' : '']" @click="changeTabType('规模')">规模</span>
				<span :class="[tabType == '类型' ? 'active' : '']" @click="changeTabType('类型')">类型</span>
			...
			<div class="chart-box">
				<el-row :gutter="20" v-if="tabType == '条件选择'">
					<el-col :span="24" style="margin-bottom: 10px">
						...
						<el-select v-model="adcd" placeholder="行政区划" @change="getTbList">
							<el-option v-for="item in adcdOption" :key="item.value" :label="item.label" :value="item.value" />
						</el-select>
					</el-col>
					<el-col :span="24" style="margin-bottom: 10px" v-if="sideType == '水源'">
						<span style="color: #fff">水源类型：</span>
						<el-select v-model="waterType" placeholder="水源类型" @change="getTbList">
							<el-option v-for="item in waterTypeOption" :key="item.value" :label="item.label" :value="item.value" />
						</el-select>
					</el-col>
					<el-col :span="24" style="margin-bottom: 10px" v-if="sideType == '水源'">
						<span style="color: #fff">水源规模：</span>
						<el-select v-model="waterScale" placeholder="水源规模" @change="getTbList">
							<el-option v-for="item in waterScaleOption" :key="item.value" :label="item.label" :value="item.value" />
						</el-select>
					</el-col>
					<el-col :span="24" style="margin-bottom: 10px" v-if="sideType == '水厂'">
						<span style="color: #fff">水厂类型：</span>
						<el-select v-model="factoryType" placeholder="水厂类型" @change="getTbList">
							<el-option v-for="item in factoryTypeOption" :key="item.value" :label="item.label" :value="item.value" />
						</el-select>
					</el-col>
					<el-col :span="24" style="margin-bottom: 10px" v-if="sideType == '水厂'">
						<span style="color: #fff">水厂规模：</span>
						<el-select v-model="factoryScale" placeholder="水厂规模" @change="getTbList">
							<el-option v-for="item in factoryScaleOption" :key="item.value" :label="item.label" :value="item.value" />
						</el-select>
					</el-col>
				</el-row>
			...
			<div class="catalog-title" ref="catalogTitle">
				<div class="type-area">
					...
					<div class="search-area" ref="searchArea">
						<div class="left-area">
							<el-input @input="getTbList" v-model="searchVal" placeholder="请输入关键字查询">
		...
		>
			<div class="dialog-content scope-content">
				<water-detail-dialog v-if="detailDialog" :sideType="sideType"></water-detail-dialog>
			</div>
		</primary-dialog>
	</div>
</template>
...
import { getWaterWorkList } from '@/api/waterWorks';
...
export default {
	...
	data() {
		return {
			...
			extraOption: {
				yAxis: {
					data: ['永嘉县', '永嘉县', '永嘉县', '永嘉县', '永嘉县', '永嘉县', '永嘉县']
				}
			},
	...
	methods: {
		...
		getTbList() {
			...
			switch (this.sideType) {
				...
				case '水厂':
					...
					getWaterWorkList({
						adcd: this.adcd,
						cwsName: this.searchVal,
						engType: this.factoryType,
						wasuRang: this.factoryScale
					}).then(res => {
						this.tbLoading = false;
						if (res.code == 0) {
							this.dataList = res.data;

							this.listTitleTotal = res.data.length;
							const pointList = this.dataList.map(item => ({
								longitude: item.cwsLong,
								latitude: item.cwsLat,
								name: item.cwsName,
								type: '水厂',
								text:`${item.cwsName || "-"} 设计供水规模:${item.desWasuScal || "-"}(吨/天)`,
								props: { 名称: item.cwsName, 覆盖人口: item.desWasuPop, '设计供水规模(吨/天)': item.desWasuScal }
							}));
							this.mapObj.drawClusterPoint(
								pointList,
								{
									scale: 0.7,
									textOffsetX: 12,
									textOffsetY: -22,
									src: require('@/assets/images/MapPoint/water-work.png'),
									srcs: require('@/assets/images/MapPoint/group/water-work.png')
								},
								'waterWorks'
							);
						}
		...
		// 点击简表行
		rowClick(data) {
			const [lng, lat] = [data.cwsLong, data.cwsLat];
