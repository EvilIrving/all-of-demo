<template>
  <div class="project-detail-dialog map-dialog">
    <div class="tap-wrap">
      <span v-for="(item, index) in list" :key="index" :class="{ active: listActive == item.name }" @click="handleMenuClick(item.name)">{{ item.name }}</span>
    </div>
    <div class="item-wrap common-item" v-if="listActive == '视频监控'" width="100%">
      <div class="left" v-if="listActive == '视频监控'">
        <el-tree class="video-tree" ref="tree" :data="videoDataList" :props="videoDefaultProps" @node-click="handleVideoClick" node-key="cameraIndexCode" icon-class highlight-current :expand-on-click-node="true">
      ...
      <div class="right">
        <div class="video-content" v-loading="videoLoading">
          <!-- rtmp视频播放 -->
          <video-player v-if="cameraSource == 2" ref="videoPlayer" :options="playerOptions" />
          <primary-video ref="videoDialog" :code="cameraCode" v-if="cameraVisible"></primary-video>
        </div>
      </div>
    </div>
    <div class="item-wrap common-item" width="100%" v-if="listActive == '建设档案'">
      <div class="left" v-if="listActive == '建设档案'">
        <ul v-if="buildDataList.length > 0">
          <li v-for="(item, index) in buildDataList" :key="index" :class="{ active: buildActive == index }" @click="handleBuildClick(item, index)">
    ...
    <div class="item-wrap common-item" width="100%" v-if="listActive == '规划档案'">
      <div class="left" v-if="listActive == '规划档案'">
        <ul v-if="buildDataList2.length > 0">
          <li v-for="(item, index) in buildDataList2" :key="index" :class="{ active: buildActive2 == index }" @click="handleBuildClick2(item, index)">
            <i :style="{
    ...
    <iframe class="common-item" v-if="listActive == '运行管理'" :src="url" frameborder="0" width="100%"></iframe>
    <iframe class="common-item" v-if="listActive == '洪水预报'" :src="floodUrl" frameborder="0" width="100%">洪水预报</iframe>
    <!-- <iframe
      class="common-item"
      ...
      >巡查</iframe
    > -->
    <iframe class="common-item" v-if="listActive == '安全监测'" :src="safeUrl" frameborder="0" width="100%">安全监测</iframe>
  </div>
</template>
        ...
        if (this.videoDataList.length > 0) {
          ...
          this.$nextTick(() => {
            if (this.videoDataList.length > 0) {
              ...
              this.cameraSource = this.videoDataList[0].cameraSource;
...
<style lang="scss">
.video-tree.el-tree--highlight-current
