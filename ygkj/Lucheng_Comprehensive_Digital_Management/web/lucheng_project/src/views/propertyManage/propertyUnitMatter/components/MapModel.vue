<!--
 * @Author: hanyu
 * @LastEditTime: 2022-06-02 09:37:35
 * @Description: 地图模式
 * @FilePath: \lucheng-appe:\lucheng\web\lucheng_project\src\views\propertyManage\propertyUnitMatter\components\MapModel.vue
-->
<template>
  <div class="map_wrap">
    ...
    <div class="popup_map" ref="popup" v-show="currentCoordinate">
      <div class="popup_content">
        ...
        <div class="popup_item">
          <p>海塘等级：</p>
          <span>{{
            popupData.seawallLength ? popupData.seawallLevel + "级" : "-"
          }}</span>
        </div>
        <div class="popup_item">
          <p>海塘长度：</p>
          <span>{{
            popupData.seawallLength ? popupData.seawallLength + "米" : "-"
          }}</span>
        </div>
        <div class="popup_item">
          <p>安全鉴定：</p>
          <span>{{
            popupData.safetyConclusion ? popupData.safetyConclusion : "-"
          }}</span>
        </div>
      </div>
    </div>
...
import { drawPoint, drawLines } from "@/utils/mapUtil";
...
export default {
  ...
  methods: {
    ...
    _drawPoints(coordinates, type) {
      ...
      if(type == '水闸'){
        styleObj.src =  require("@/assets/images/sluice-icon.png")
      } else if(type == '堤防'){
        styleObj.src =  require("@/assets/images/dike.png")
      }
