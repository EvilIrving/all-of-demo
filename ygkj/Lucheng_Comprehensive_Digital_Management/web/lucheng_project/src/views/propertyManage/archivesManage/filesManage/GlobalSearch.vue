<template>
  <div class="globalSearch_box">
    <div class="file_wrap global_search" v-if="condition == 'fileManage'">
      <div class="right_top">
        <div class="table_input">
          <span class="global_search" @click="returnFileManage"
            ><i></i>返回</span
          >
        ...
        >
          ...
          <el-table-column prop="collectionFlag" label="收藏状态">
            <template slot-scope="scope">
              <span
                v-if="scope.row.collectionFlag == 0"
                class="unfavorite"
                @click="unfavorite(scope.row.id)"
                ><i></i>已收藏</span
              >
              <span
                v-if="scope.row.collectionFlag == 1"
                class="collectFile"
                @click="collectFile(scope.row.id)"
                ><i></i>收藏</span
              >
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    returnFileManage() {
      this.$router.push("/propertyManage/filesIntegration");
    },
    editFile(id) {
      //文件编辑
...
.global_search .table_input .el-input__inner {
  width: 300px;
}
.file_wrap .tabBlock {
  border: none;
