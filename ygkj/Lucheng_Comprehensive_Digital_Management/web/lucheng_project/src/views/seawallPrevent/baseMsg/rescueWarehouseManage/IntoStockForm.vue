        ...
        <el-row>
          ...
          <el-col :span="12" v-if="formData.type==='1'">
            ...
            <el-form-item label="采购凭证：" >
              <el-button type="text" @click="openFile" v-if="id">点击查看</el-button>
              <unpload-file
                  v-else
          ...
          <el-table :data="outboundList" border style="width: 100%">
            ...
            <el-table-column label="物资" align="center">
              <template slot-scope="scope">
                <el-select
                    v-model="outboundList[scope.$index].value1"
                    clearable
                    style="width: 100%;"
                    v-if="scope.row.switchChange"
                    placeholder="请选择物资">
                  <el-option
                      v-for="item in inboundMaterialsList"
                      :key="item.label"
                      :value="item.label"
                      :label="item.label"
                  />
            ...
            <el-table-column label="数量" align="center">
              <template slot-scope="scope">
                <el-input v-model="outboundList[scope.$index].value2" placeholder="请输入"
            ...
            <el-table-column label="登记人" align="center">
              <template slot-scope="scope">
                <el-input v-model="outboundList[scope.$index].value6" placeholder="请输入"
                          v-if="scope.row.switchChange"></el-input>
...
export default {
  ...
  methods: {
    openFile(){
      ...
      window.open(this.formData.fileList[0].fileUrl)
    },
    ...
    async getDetail() {
      ...
      if (res.code == 0) {
        ...
        this.fileList = res.data.list[0].fileList
        console.log(res.data.list[0].fileList)
      }
    },

    submitForm(formName) {
      this.formData.num = Number(this.formData.num)
      this.formData.univalent = parseFloat(this.formData.univalent)
    ...
    async save(data) {
      ...
      let res = await intoOutScrappedApi.intoStockAdd(data)
      if (res.code == 0) {
        this.$message({
          message: '保存成功',
          type: 'success',
