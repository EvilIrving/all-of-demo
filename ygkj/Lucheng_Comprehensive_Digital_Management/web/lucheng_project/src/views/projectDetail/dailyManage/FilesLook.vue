    ...
    <div class="file_wrap" v-if="condition == 'fileManage'">
      ...
      <div class="file_wrap_right">
        <div class="right_top">
          <div class="table_input">
            <!-- <span
              ...
              ><i></i>文件上传</span
            > -->
            <!-- <span class="file_upload" @click="fileUpload()"
              ><i></i>文件上传</span
            >
            <span class="global_search" @click="globalSearch"
              ><i></i>全局搜索</span
            > -->
            <div style="float: right">
              <primary-table ref="fileTable" title="" :hideAdd="true" @doSearch="doSearch">
              </primary-table>
            </div>
          </div>
        ...
        <div class="right_mid">
            ...
            <div class="filter_box" v-show="filterShow">
              <div class="filter_item" :class="[
                ...
                <el-checkbox-group v-model="checkboxGroup1" ref="tagTypeRef" @change="getFileList">
                  <el-checkbox-button v-for="tag in tagOptions" :label="tag.id" :key="tag.id">{{ tag.name }}</el-checkbox-button>
                </el-checkbox-group>
                <i v-show="tagHeight > 42" :class="tagShow ? 'active' : ''" @click="tagShow = !tagShow"><b v-if="tagShow">收起</b><b v-if="!tagShow">展开</b></i>
              </div>
              <div class="filter_item">
                <span>文件类型:</span>
                <el-checkbox-group v-model="checkboxGroup2" @change="getFileList">
                  <el-checkbox-button v-for="item in fileTypeOptions" :label="item.value" :key="item.value">{{ item.name }}</el-checkbox-button>
                </el-checkbox-group>
              </div>
              <div class="filter_item">
                <span>时间选择:</span>
                <el-checkbox-group v-model="checkboxGroup3" @change="getFileList">
                  <el-checkbox-button v-for="year in yearOptions" :label="year" :key="year">{{ year }}</el-checkbox-button>
                </el-checkbox-group>
              </div>
            </div>
          </el-collapse-transition>
          <div class="filter_arrow">
            <span @click="filterShow = !filterShow">
              <i :class="!filterShow ? 'active' : ''"></i><b v-if="filterShow">收起</b><b v-if="!filterShow">展开</b>
            </span>
          </div>
        </div>
        ...
        <div ref="tableBox" class="table-box table_wrap">
          <el-table v-loading="loading" :data="tableData" stripe style="width: 100%" class="deep-table" border :header-row-style="{ height: '43px' }">
            <el-table-column type="index" label="序号" width="50"></el-table-column>
            <el-table-column prop="collectionFlag" label="收藏状态" width="100">
              <template slot-scope="scope">
                <span v-if="scope.row.collectionFlag == 0" class="unfavorite" @click="unfavorite(scope.row.id)"><i></i>已收藏</span>
                <span v-if="scope.row.collectionFlag == 1" class="collectFile" @click="collectFile(scope.row.id)"><i></i>收藏</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="fileName" label="文件名称">
              <template slot-scope="scope">
                <span v-if="
                  ...
                  " style="color: #0392fe; cursor: pointer" @click="lookFile(scope.row.fileUrl, scope.row.id)">{{ scope.row.fileName }}</span>
            ...
            <el-table-column label="操作" width="150">
              <template slot-scope="scope">
                <!-- <span class="edBtn" @click="editFile(scope.row.id)">编辑</span> -->
                <!-- <span
                  class="edBtn"
                ...
                <span
                  ...
                  >删除</span
                > -->
                <span class="edBtn" @click="uploadFile(scope.row.id, scope.row.fileUrl)">下载</span>
                <!-- <span class="edBtn" @click="deleFile(scope.row.id)">删除</span> -->
              </template>
            </el-table-column>
          </el-table>
          <div class="pagination_area">
            <el-pagination class="right" background layout="total,prev, pager, next" :total="total" :page-sizes="[10, 20, 30, 40]" @size-change="handleSizeChange" @current-change="handleCurrentChange"></el-pagination>
          </div>
        </div>
      </div>

      <!-- 上传文件dialog -->
      <el-dialog class="list-dialog" :visible.sync="fileUploadDialog" width="50%" :before-close="handleClose" :close-on-click-modal="false">
        <div slot="title" class="el-dialog__title">
          <strong>上传文件</strong>
        </div>
        <fileManage-form ref="fileManageForm" @cancelDialog="handleClose"></fileManage-form>
      </el-dialog>

      <!-- 文件编辑dialog -->
      <el-dialog class="list-dialog" :visible.sync="fileEditDialog" width="50%" :before-close="editClose" :close-on-click-modal="false">
        <div slot="title" class="el-dialog__title">
          <strong>文件编辑</strong>
        </div>
        <fileManage-edit ref="fileManageEdit" @cancelDialog="editClose"></fileManage-edit>
      </el-dialog>
    </div>
...
import FileTree from "../../propertyManage/archivesManage/filesManage/components/FileTree";
import PrimaryTable from "../../propertyManage/archivesManage/primary_table/index";
import FileManageForm from "../../propertyManage/archivesManage/filesManage/components/FileManageForm";
import FileManageEdit from "../../propertyManage/archivesManage/filesManage/components/FileManageEdit";
import FileLog from "../../propertyManage/archivesManage/filesManage/FileLog";
import {
  lookFileLogApi,
  getLabelDataApi,
  getFileTypeDataApi,
    ...
    .table_wrap {
      ...
      .unfavorite {
        ...
        i {
          ...
          margin-top: -2px;
          margin-right: 3px;
          background: url("../../../assets/images/unfavorite_icon.png")
            no-repeat center;
          background-size: 100% 100%;
        }
      }
      .collectFile {
        ...
        i {
          ...
          margin-top: -2px;
          margin-right: 3px;
          background: url("../../../assets/images/collectFile_icon.png")
            no-repeat center;
          background-size: 100% 100%;
        }
      }
