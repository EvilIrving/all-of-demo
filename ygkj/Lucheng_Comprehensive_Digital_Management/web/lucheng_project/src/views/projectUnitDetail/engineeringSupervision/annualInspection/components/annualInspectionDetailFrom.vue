<!--
 * @Author: wanxin
 * @LastEditTime: 2022-05-21 11:35:18
 * @Descripttion: 
 * @FilePath: \lucheng_project\src\views\projectUnitDetail\engineeringSupervision\annualInspection\components\annualInspectionDetailFrom.vue
-->
<template>
  <div>
    <div class="wrap">
    ...
    <div class="wrap">
      ...
      <div class="file_box">
        <div v-for="item in aidFiles">
          <div class="fileStyle">
            1.{{ item.fileName }}<span @click="download(item)">下载</span
            ><span @click="lookFile(item)">查看</span>
          </div>
        </div>
        <div v-for="item in runActionRecordFiles">
          <div class="fileStyle">
            2.{{ item.fileName }}<span @click="download(item)">下载</span
            ><span @click="lookFile(item)">查看</span>
          </div>
        </div>
        <div v-for="item in projCheckRecordFiles">
          <div class="fileStyle">
            3.{{ item.fileName }}<span @click="download(item)">下载</span
            ><span @click="lookFile(item)">查看</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      checkPhotoFiles: [],
      aidFiles: [],
      runActionRecordFiles: [],
      projCheckRecordFiles: [],
      fileDialog: false,
    }
  },
  components: {
  ...
  methods: {
    async getDetail() {
      let res = await annualInspectionApi.bhLgMpAcList({
      ...
      if (res.code == 0) {
        ...
        this.date = data.tm
        this.num = data.hidCount
        this.people = data.joinUser
        this.checkPhotoFiles = data.checkPicList
        this.aidFiles = data.aidFileList
        this.runActionRecordFiles = data.runActionRecordFiles
        this.projCheckRecordFiles = data.projCheckRecordFiles
