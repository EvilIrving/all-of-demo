<!--
 * @Author: wanxin
 * @LastEditTime: 2022-05-20 20:35:03
 * @Descripttion: 
 * @FilePath: \lucheng_project\src\views\projectUnitDetail\engineeringSupervision\annualInspection\components\annualInspectionAddFrom.vue
-->
<!--
 * @Author: wanxin
...
<template>
  <div>
    ...
    >
      ...
      <el-form-item label="带队检查人：" prop="checkUid">
        <el-input
          class="form-item-value"
          type="textarea"
          v-model="ruleForm.checkUid"
        ></el-input>
      </el-form-item>
      ...
      <el-form-item label="上传附件：">
        ...
        <uploadFile
          accept=".pdf"
          @success="handleSuccessBg"
          @remove="handleRemoveBg"
          @review="handlePreviewBg"
          :fileList="fileListBg"
          :size="15"
        />
      </el-form-item>
      <el-form-item label="上传附件：">
        <div>年度检查工程运行管理检查记录表</div>
        <uploadFile
          accept=".pdf"
...
export default {
  ...
  methods: {
    ...
    async getLoadPageDisDom() {
      ...
      if (res.code == 0) {
        ...
        this.ruleForm = {
          ...
          aid: data.aid,
          runActionRecord: data.runActionRecord,
        ...
        this.fileListBg = data.aidFileList.map((item) => {
          return {
            name: item.fileName,
            url: item.fileUrl,
          }
        })
        this.fileListGl = data.runActionRecordFiles.map((item) => {
          return {
            name: item.fileName,
            url: item.fileUrl,
          }
    ...
    handleSuccessGl(data) {
      if (data.file.code == 0) {
        let files = data.file.data
        this.ruleForm.runActionRecord = files.id
        this.fileListGl = [
          {
            name: files.originalName,
            url: files.filePath,
