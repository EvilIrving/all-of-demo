<!--
 * @Author: wanxin
 * @LastEditTime: 2022-05-21 11:33:12
 * @Descripttion: 
 * @FilePath: \lucheng_project\src\views\projectUnitDetail\engineeringSupervision\preFloodInspection\components\preFloodInspectionDetailFrom.vue
-->
<template>
  <div>
    <div class="wrap">
    ...
    <div class="wrap">
      ...
      <div class="file_box">
        <div v-for="item in aidFiles">
          <div class="fileStyle">
            1.{{ item.fileName }}<span @click="lookFile(item)">查看</span>
          </div>
        </div>
        <div v-for="item in projCheckRecordFiles">
          <div class="fileStyle">2.{{ item.fileName }}<span @click="lookFile(item)">查看</span></div>
        </div>
      </div>
    </div>
    <el-dialog
