...
export default {
  ...
  methods: {
              ...
              case '水闸工情':
                {
                  if (properties.data.upz) {
                    ...
                    let controlGateFlow = properties.data.controlGateFlow ? properties.data.controlGateFlow : '-';
                    obj = {
              ...
              case '积水':
                {
                  obj = {
                    名称: properties.data.name,
                    积水: properties.data.waterLevel + properties.data.waterLevelUnit
                  };
                }
                break;
              case '视频':
    ...
    getInScope() {
      ...
      let val = this.map.getLayers().getArray()[this.map.getLayers().getArray().length - 1].values_.source.uidIndex_;
      console.log('getInScope', val, val[Object.keys(val)[0]]);
      val =
        typeof val[Object.keys(val)[0]] == 'object' ? val[Object.keys(val)[0]].values_.geometry.flatCoordinates : '';
    ...
    //高亮显示主城区划
    showCityAreaLine() {
      const searchUrl = 'http://112.17.127.75:8008/arcgis/rest/services/wenzhou/wzmcls/MapServer/2/query';
      const opt = {
        where: 'objectid<=4',
        inSR: 4490,
        outFields: '*',
    ...
    drawLineEx(coordinates, styleObj = {}) {
      ...
      selectClick.on('select', (e) => {
        ...
        if (features.length > 0) {
          // console.log(features[0].values_)
          this.$emit('showProjectDialog', features[0].values_.data, features[0].values_.data);
        }
      });
      this.map.addInteraction(selectClick);
    },
    ...
    showAnalysisPoints(data, type) {
      ...
      const stationTypeArr = ['水库水位', '河道水位', '堰闸水位', '潮位', '闸门开度', '水文站水质', '积水'];
