 ...
 * @Description: 水位图
 * @LastEditors: wqn
 * @LastEditTime: 2022-05-18 18:06:54
 * @FilePath: \lucheng\src\views\ourPujiang\WaterChart.vue
-->
<template>
      ...
      <ul class="info">
        ...
        <li v-for="(item,index) in highWaterSurfaceData" :style="{left: index/(highWaterSurfaceData.length - 1)*100 + '%'}" :key="`${item.hightWaterLevel}-${index}`">
          ...
          <div class="discovery-time">
            <div>{{item.discoveryTime ? item.discoveryTime.split(' ')[0].substring(5).replaceAll('-', '.') : '--'}}</div>
            <div>{{item.discoveryTime ? item.discoveryTime.replaceAll('-', '.').split(' ')[1].substring(0, 5) : '--'}}</div>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  watch: {
    ...
    dateValue(val) {
      ...
      this.getEChartsData(time.replaceAll('.', '-') + ':00');
    }
  },
  created() {
    const d = new Date();
  ...
  mounted() {
    ...
    this.getEChartsData(this.endDateTime.replaceAll('.', '-') + ':00');
  },
  destroyed() {
    clearInterval(this.timer);
  },
  methods: {
     ...
     */
    getEChartsData(time) {
      sectionInformationHydrograph({ time: time }).then((res) => {
        this.xAxisData = [];
        this.distanceData = [];
        this.groundData = [];
