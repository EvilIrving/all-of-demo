	...
	<div>
		...
		<div class="collapse-panel__border" style="width: 490px">
			<div class="collapse-panel__content">
				...
				<!-- 山洪 -->
				<panel-mountain-torrents @legend-change="legendChange" />
			</div>
		</div>
		<div class="collapse-panel__border" style="width: 490px; margin-left: 0.5rem">
			<div class="collapse-panel__content">
				<!-- 1h, 3h, 6h, 12h, 24h, 72h 实时降雨量 -->
				<panel-item :inSimulation="previewModule" title="实时降雨" hasSwitch>
					<!-- 预报降雨量 标题右侧图标点击事件 -->
					<template #title-switch>
						<!-- v-if="cardSelect == 'realtime'" -->
						<i title="模拟演练" style="height: 18px" :class="['icon-moniyanlian', 'icon-sty']" @click="doDrill()"> </i>
						<i
							title="等值面"
							style="height: 18px"
							@click="clickRealtimeRainfull('realdzmbtn')"
						...
						/>
						<i v-show="!previewModule" @click="goToAMap('雨量')" title="一张图" class="icon-yizhangtu icon-sty" />
					</template>
					<!-- 雨量站 点位站 -->
		...
		>
			<div class="dialog-content">
				<div class="primary-dialog-tabs">
					<span v-for="item in tabsArr" :key="item" :class="tabActive == item ? 'active' : ''">{{ item }}</span>
				</div>
				<div class="content-box">
					...
					<div class="search-row">
						...
						>
							<el-option v-for="item in historyTyphoonList" :key="item.value" :label="item.label" :value="item.value">
							</el-option>
						</el-select>
					</div>
				</div>
...
export default {
	...
	computed: {
		...mapGetters(["ScreenMap", "currentSelectArea", "previewModule", "formatterCurrentTime", "rainfallArea"]),
		rainfallPointNum() {
	...
	methods: {
		...
		typhoonChange(val) {
			const arr = this.historyTyphoonList.filter(item => {
				return item.id == val;
			});
		...
		doPreview() {
			if (this.typhoonVal) {
				...
				let startTime = this.typhoonData.beginTime ? new Date(this.typhoonData.beginTime).getTime() : "";
				let endTime = this.typhoonData.endTime ? new Date(this.typhoonData.endTime).getTime() : "";
				this.setPreviewData(this.typhoonData);
				this.setStartTimeatmap(startTime);
				this.setEndTimeatamp(endTime);
				let preStartTime = this.typhoonData.futureBegin ? new Date(this.typhoonData.futureBegin).getTime() : "";
				let preEndTime = this.typhoonData.futureEnd ? new Date(this.typhoonData.futureEnd).getTime() : "";
