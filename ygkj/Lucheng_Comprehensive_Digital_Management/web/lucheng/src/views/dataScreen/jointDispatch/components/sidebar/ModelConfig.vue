...
export default {
  ...
  mounted() {
    ...
    this.getDetail(sTime,eTime,1.00,this.info.limitLevel,1.00);
  },
  methods: {
    async getDetail(sTime,eTime,flowZoom,limitWater,outflowZoom) {
      this.tableData = [];
      ...
      await axios({
        ...
        }&startTime=${sTime}&endTime=${eTime}&flowZoom=${flowZoom}&limitWater=${limitWater}&jsonFloodArray=[]&jsonWaveArray=[]&jsonSplitWaveArray=[]&outflowZoom=${outflowZoom}&outflowArray=[]`,
        method: "get",
      }).then((res) => {
        if (res.data.success) {
          const info = res.data;
