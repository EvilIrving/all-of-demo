...
<template>
  <div class="sidebar-box">
    <div class="sidebar-head">
      <span>模拟结果</span>
      <i class="icon-simulation-setting" @click="modelDispatchOpen"></i>
    </div>
    <ul class="model-info">
    ...
    <div class="sidebar-head">
      ...
      <span>
        ...
        <i class="icon-bar-count" @click="dispatchOpen"></i>
      </span>
    </div>
    ...
    ></dispatch-dialog>
    <!-- 联合调度 -->
    <model-dispatch-dialog
      v-if="modelDispatchShow"
      :dailogShow="modelDispatchShow"
      @handleClose="modelDispatchClose"
    ></model-dispatch-dialog>
  </div>
</template>
...
import ModelDispatchDialog from "../ModelDialog2";
...
export default {
  ...
  components: { DispatchDialog, ModelDispatchDialog },
  data() {
    return {
      name: "",
      startRange: null,
  ...
  methods: {
    ...
    modelDispatchOpen() {
      this.modelDispatchShow = true;
    },
    modelDispatchClose() {
      this.modelDispatchShow = false;
    },
    async getDetail() {
      this.tableData = [];
      ...
      let eTime =
        this.$moment().subtract(2, "days").format("YYYY-MM-DD HH") + ":00:00";
      await axios({
        url: `https://flood.ygwjg.com/mgt/bus/floodDispatch/dispatch?token=${
          this.tokenObj[this.info.projectName]
        }&startTime=${sTime}&endTime=${eTime}&flowZoom=1.00&limitWater=500.0&jsonFloodArray=[]&jsonWaveArray=[]&jsonSplitWaveArray=[]&outflowZoom=1.00&outflowArray=[]`,
        method: "get",
      }).then((res) => {
        if (res.data.success) {
          const info = res.data;
