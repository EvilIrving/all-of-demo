 ...
 * @LastEditTime: 2022-08-29 15:17:08
 * @LastEditors: Please set LastEditors
 * @Description: 实时降雨小组件
 * @FilePath: \lucheng\src\views\water_disaster_custom\cards\RealRainCard.vue
-->
<template>
  <div class="rain-card">
...
import FloodAnalysisRightVue from '@/views/dataScreen/floodJudgeNew/components/sidebar/FloodAnalysisRight.vue';
export default {
  name: 'RealRainCard',
  layerName: ['maxPointLayer', 'rainLayer', 'isolineLayer', 'rainfall'],
  ...
  beforeDestroy() {
    clearInterval(this.timer);
    this.ScreenMap.loadRainfallPoint([], FloodAnalysisRightVue);
  ...
  methods: {
    ...
    precipitationCardClick(val) {
      if (val == null) {
        ...
        this.ScreenMap.removeLayerByName('isolineLayer');
    ...
    async getIsohyetArea(opt, type) {
      ...
      if (type) {
        areaUrl = mapConfig.mapServer + 'gps/dengzhiPIDWReal1012/GPServer/dengzhiPIDWReal1012/submitJob';
      }
      await axios({
        url: areaUrl,
        method: 'post',
        ...
        let interVal = setInterval(async () => {
          let url =
            mapConfig.mapServer +
            '/gps/PointTRaster/GPServer/PointTRaster/jobs/' +
            res.data.jobId +
            '/results/Idwres?f=pjson';
          if (type) {
            url =
              mapConfig.mapServer +
              'gps/dengzhiPIDWReal1012/GPServer/dengzhiPIDWReal1012/jobs/' +
              res.data.jobId +
              '/results/Idwres?f=pjson';
          }
          await axios({
            ...
            if (!result.data.value.mapImage.error) {
              ...
              _this.getIsoline(res.data.jobId);
            }
          });
        }, 1000);
      });
    ...
    async getTextPoint(jobId) {
      let url = mapConfig.mapServer + `gps/DZPIDWFurLabel/MapServer/jobs/${jobId}/1/query?where=1=1&outFields=*&f=json`;
      await axios({
        url: url,
        method: 'get'
      }).then(async (result) => {
    ...
    async getIsoline(jobId) {
      let url =
        mapConfig.mapServer + `gps/dengzhiPIDWReal1012/MapServer/jobs/${jobId}/1/query?where=1=1&outFields=*&f=json`;
      await axios({
        url: url,
        method: 'get'
      }).then(async (result) => {
        this.ScreenMap.showIsoline(result.data);
      });
    },
    async getIsohyetOpacity(xmin, ymin, xmax, ymax, jobId, type) {
      ...
      let url = `${mapConfig.mapServer}gps/dengzhiPIDWReal1012/MapServer/jobs/${jobId}/export`;
      if (type == 'forecast') {
    ...
    loadRainfallPoint() {
      if (this.ylbtn) {
        this.ScreenMap.loadRainfallPoint(this.rainList, true);
        this.$emit('legend-change', '预报降雨量', {});
        this.$emit('legend-change', '实时降雨量', {
          label: '实时降雨量',
          children: [
