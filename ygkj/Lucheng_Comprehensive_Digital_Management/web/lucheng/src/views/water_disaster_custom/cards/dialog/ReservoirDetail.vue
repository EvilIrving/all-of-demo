<template>
  <div class="primary-dialog reservoir-detail-table">
    ...
    >
      <template #stat>
        <StatisticsBox title="救援队伍总数" icon="icon-shu-liang-tong-ji">
          <span>{{ statisticsData.total || '-' }} </span> 座
        </StatisticsBox>
        <StatisticsBox title="XXXXXXXX队(最多)" icon="icon-xu-shui-lv">
          <span>{{ statisticsData.avgStorageRatio || '-' }} </span> %
        </StatisticsBox>
        <StatisticsBox title="XXXXXXXX队(最少)" icon="icon-na-xu-nengli">
      ...
      <template #query>
        <LabelFormItem label="抢险专业：">
          <el-select v-model="areaCode" size="small" clearable placeholder="全部" @change="getList">
            <el-option v-for="item in areaList" :key="item.name" :label="item.name" :value="item.name"></el-option>
          </el-select>
        </LabelFormItem>
        ...
        <LabelFormItem label="关键字:">
          <el-input v-model="resName" placeholder="请输入水库名称" style="width: 180px" />
        </LabelFormItem>
...
export default {
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      const { resName, pageSize, pageNum, orderColumn, ascOrDesc, type } = this;
      const opt = {
        resName,
        resAdcd: this.areaCode,
        resBas: this.basin,
        exportStrategy: type === '大中型水库' ? 1 : 0,
        drainage24H: type === '大中型水库',
        drainage48H: type === '大中型水库',
        drainage72H: type === '大中型水库',
        pageSize,
        pageNum,
        orderColumn,
        ascOrDesc
