	...
	<div class="water-card">
		<panel-item title="水源供水" tc @tclick="tclick">
			<div class="charts-panel">
				<!-- 水源地 -->
				<sub-title-click-card title="水源地" type="radio" :radioList="['数量', '人口']" @radioChange="areaChange">
					<div slot="title-operate">
						<i
							title="地图落点"
							:class="[sydbtn == true ? 'icon-yinzhang-active' : 'icon-yinzhang-1', 'icon-sty']"
							@click="drawPointArea('sydbtn')"
						/>
						<i title="查看详表" class="icon-liebiao icon-sty" @click.stop="openDetailsDialog('水源地')" />
					</div>
					<div class="chart-panel">
						<div class="chart-tab">
							<span :class="['chart-tab-item', areaType == 0 ? 'active' : '']" @click="areaTypeChange(0)">按规模</span>
							<span :class="['chart-tab-item', areaType == 1 ? 'active' : '']" @click="areaTypeChange(1)">按类型</span>
						</div>
						<div class="pie-box">
							...
							<div class="legend-panel">
								<div class="legend-item" v-for="(item, index) in pieChartData" :key="index">
									<div>
										...
										<span class="text" :style="{ width: areaType == 0 ? '100px' : '60px' }">{{ item.name }}</span>
									</div>
									<div v-if="areaType == 0">
				...
				<!-- 水量预警 -->
				<sub-title-click-card title="水量预警" type="radio" :radioList="['数量', '人口']" @radioChange="warnChange">
					<div slot="title-operate">
						<i
							title="地图落点"
						...
						/>
						<i title="查看详表" class="icon-liebiao icon-sty" @click.stop="openDetailsDialog('水量预警')" />
					</div>
					<div class="chart-panel">
						<div class="count-box">
							<div class="all-count">
...
export default {
	...
	data() {
		return {
			timer: null,
			statisticsData: [{ num: 0 }, { num: 0 }, { num: 0 }, { num: 0 }],
	...
	methods: {
		...
		areaChange(val) {
			...
			this.areaType == 0 ? this.statisSwhsNumByScale() : this.statisSwhsNumByType();
			this.statisSwhsNumByArea();
		},
		// 数量 人口 tab切换  水量预警
