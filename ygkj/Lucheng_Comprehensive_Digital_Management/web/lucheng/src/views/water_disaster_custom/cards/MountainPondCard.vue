<template>
  <div class="Mountain-Pond-Card" style="">
    <panel-item
      title="山塘"
      has-switch
      tc
    ...
    >
      ...
      <template #title-switch>
        ...
        <i
          ...
          @click.stop="openDetailsDialog('山塘')"
      ...
      <div class="is-water-box" v-if="activeTab == 0">
        ...
        <div class="pie_box">
          ...
          <div id="mountainPie" style="width:50%;height:100%" ref="mountainPie"></div>
...
export default {
  ...
  computed: {
    ...mapGetters(["previewModule", "SetMountainPondCount"]),
    map() {
  ...
  mounted() {
    this.$nextTick(() => {
      this.getPondLoggingList(); //地图点位获取
      this.getstatisticalProjectNum(); //tab1山塘数量统计
      // this.getWaterLoggingList();
      this.mnpdProject(); //tab1饼图统计
      this.getstatisticalDiskNumByArea(); //tab2山塘行政区划数量统
  ...
  methods: {
    ...
    async getstatisticalProjectNum() {
      await disasterPreventionApi
        .getstatisticalProjectNum({ type: "山塘" })
        .then((res) => {
          if (res.code == 0) {
            this.allpondnum = res.data;
          }
        });
    },
    mountainPie() {
      var chartDom = document.getElementById("mountainPie");
    ...
    // 获取点位
    async getPondLoggingList() {
      let res = await disasterPreventionApi.mnpdProject();
      this.pondStList = res.data.list;
      // console.log(this.pondStList,987987)
    },
    // 地图点位
    handleStagnantWater(btnType) {
      ...
      let style2 = {
        ...
        anchor: [27, 30],
        anchorXUnits: "pixels",
        src: require("@/assets/images/pond2big.png"),
        srcs: require("@/assets/images/pond2big-juhe.png"),
        // srcs:require("@/assets/images/MapPoint/sluice.png")
      };
      let style3 = {
        scale: 0.7,
        anchor: [27, 30],
        anchorXUnits: "pixels",
        src: require("@/assets/images/pond3big.png"),
        srcs: require("@/assets/images/pond3big-juhe.png"),
        // srcs:require("@/assets/images/MapPoint/sluice.png")
      };
      let pointList1 = [];
      let pointList2 = [];
      ...
      let pointList = [];
      for (let point of this.pondStList) {
        // let imgSrc = require("@/assets/images/pond1big.png");
        // if (point.mnpdCate == "普通山塘") {
        //   imgSrc = require("@/assets/images/pond2big.png");
        // }
        // if (point.mnpdCate == "高坝山塘") {
        //   imgSrc = require("@/assets/images/pond3big.png");
        // }
        let obj = {
          //    lng: Number(point.dtMidLong && point.dtMidLong.split(",")[0]),
          //       lat: Number(point.dtMidLat && point.dtMidLat.split(",")[1]),
          lng: point.dtMidLong,
          lat: point.dtMidLat,
          type: "山塘",
          src: style1.src,
        ...
        if (point.mnpdCate == "屋顶山塘") {
          pointList1.push(obj);
        } else if (point.mnpdCate == "普通山塘") {
          pointList2.push(obj);
        } else if (point.mnpdCate == "高坝山塘") {
          pointList3.push(obj);
        }
      }
      ...
      if (this[btnType]) {
        ...
        this.$emit("legend-change", "山塘", {
          label: "山塘",
          clusterLayer:['pointLayer1','pointLayer2','pointLayer3'],
          children: [
            {
              label: "高坝山塘",
              icon: "icon-pond1big",
    ...
    async getstatisticalDiskNumByArea() {
      await disasterPreventionApi
        .getstatisticalDiskNumByArea({ type: "山塘" })
        .then((res) => {
          if (res.code == 0) {
            let rows = [];
...
<style lang="scss">
.Mountain-Pond-Card .panel-item .panel-item__content--tabs .tab-button,
.Mountain-Pond-Card .panel-item .panel-item__title--tabs .tab-button {
  display: inline-block;
  font-size: 16px;
  padding: 0 12px;
}
.Mountain-Pond-Card .panel-item .panel-item__content--tabs .tab-button.active,
.Mountain-Pond-Card .panel-item .panel-item__title--tabs .tab-button.active {
  padding: 0 12px;
}
</style>
<style lang="scss" scoped>
