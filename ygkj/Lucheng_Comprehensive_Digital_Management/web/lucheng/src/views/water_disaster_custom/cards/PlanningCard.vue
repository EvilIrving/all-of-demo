	...
	<div class="planning-card">
		<page-pane>
			...
			<template #append>
				...
				<i class="icon-liebiao" @click="openDetailsDialog" />
		...
		<PlanningDetail :visible.sync="detailVisible" :title="titleName" :ptId="titleId" />
	</div>
</template>
<script>
...
import PlanningDetail from "./dialog/PlanningDetail.vue";
import { log } from "easemob-webrtc";
const RingOptions = {
	grid: {
		top: 40,
		bottom: "2%",
...
export default {
	...
	legendName: "规划",
	components: { PagePane, PagePaneItem, TableTitle, PlanningDetail },
	data() {
		return {
			detailVisible: false,
			titleOption: [],
			...
			projectOption: { ...RingOptions, tooltip: { confine: true, formatter: "{b}:{c}个" } },
			projectColors: [],
			...
			investOption: { ...RingOptions, tooltip: { confine: true, formatter: "{b}:{c}万元" } },
			investColors: [],
	...
	computed: {
		...
		titleName() {
			return this.titleOption.find(item => item.id == this.titleId)?.planName || "";
		}
	},
	mounted() {
	...
	methods: {
		...
		afterFinishRateConfig(options) {
			...
			const color = genBodyLinearGradients("#2886c6", "#50bfda");
			const series = generateSeries({ data, color: color });
			if (options.series[1].data[0]) {
				const data2 = options.series[1].data;
				const series2 = generateSeries({ data2, color: color });
		...
		async getStatsOfDevPlan() {
			await planDigitizationApi.getStatsOfDevPlan({ ptId: this.titleId }).then(res => {
				if (res.code == 0) {
					let {
						investFinishRate4County,
						projStat4PlanRec,
						investTotal4County,
						investFinishRateLast = null,
						...rest
					} = res.data;
					this.finishRateData.rows = investFinishRate4County.map((item, index) => ({
						name: item.areaName,
						data1: parseInt(item.rate).toFixed(1),
						data2: investFinishRateLast ? investFinishRateLast?.[index] : 0
					}));
