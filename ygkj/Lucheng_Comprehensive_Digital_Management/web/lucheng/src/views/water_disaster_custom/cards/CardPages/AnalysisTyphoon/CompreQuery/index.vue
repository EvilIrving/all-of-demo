<template>
  <div class="comprehensive">
    <el-row>
      <el-col :span="advanced ? 24 : 12" :class="{ 'justify-none': advanced }">
        <span style="white-space: nowrap">影响温州：</span>
        <el-select style="width: 110px" v-model="affectedWz" @change="baseQuery">
      ...
      <el-col v-if="!advanced" :span="11.5">
        ...
        <el-select style="width: 110px" v-model="year" clearable @change="baseQuery">
    ...
    <el-row>
      <el-col :span="12">
        <span @click="advanced = !advanced" :class="[advanced ? 'advanced' : 'base', 'query']"
          >{{ advanced ? '高级查询' : '基本查询' }} <i class="el-icon-caret-bottom"
        /></span>
      </el-col>
      <el-col :span="12">
        <span :class="['risk-area', riskType == true ? 'active' : '']" @click="riskClick">台风风险</span>
      </el-col>
    </el-row>
    ...
    <div ref="typhoonTable" style="height: 100%">
      ...
      >
        ...
        <u-table-column label="编号" width="60" prop="code">
          <template slot-scope="{ row }">
            ...
            <i
              ...
              title="70%概率风圈"
              @click="showTyphoonCircle(row.code)"
            />
          </template>
        </u-table-column>
        <u-table-column label="中文名" prop="name" width="60" />
...
export default {
  ...
  methods: {
    ...
    showTyphoonCircle(code) {
      this.analysisPage.$emit('showTyphoonCircle', code);
    },
    rowClick(row, column) {
          ...
          for (let i = 0; i < this.typhoonList.length; i++) {
            ...
            if (this.typhoonList[i].current) {
              this.$nextTick(() => {
                ...
                this.analysisPage.$emit('rowClick', {
                  code: this.typhoonList[i].code,
                  name: this.typhoonList[i].name,
                  firstFlag: true
                });
              });
              break;
