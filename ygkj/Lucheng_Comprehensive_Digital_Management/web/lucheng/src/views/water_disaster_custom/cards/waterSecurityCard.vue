...
<template>
	<div class="water-security-card">
		...
		<WaterSecurityDetail :visible.sync="detailVisible" />
	</div>
</template>
...
import WaterSecurityDetail from "./dialog/WaterSecurityDetail.vue";
...
export default {
	...
	methods: {
		...
		handleMapNet(btnType) {
			...
			// let normalSrc = require("@/assets/images/shuiyuandi-ld-5.png");
			let pointList = this.reservoiList.map(point => {
				let imgSrc = require("@/assets/images/shuiyuandi-ld-5.png");
				// if (point.overLimitVal != null && point.overLimitVal > 0) {
				//   imgSrc = require("@/assets/images/shuiku-linshi.png");
				// }
				return {
					name: point.name,
					type: "水源保障",
