		...
		<page-pane title="数据分布" @onTitle="() => {}">
			...
			<div class="custom-legend-chart">
				...
				<div class="custom-legend" style="height: 180px">
					<ul class="list">
						<li v-for="(item, index) in distributedData.rows" :key="index" class="list-item">
							<i class="dot" :style="{ backgroundColor: afterDistributedColor[index] }" />
			...
			<table-title title="水利基础数据">
				<template #append>
					<el-select v-model="basicTab" popper-class="styled-select" class="styled-select" @change="onCategoryChange">
						<el-option v-for="(item, index) in basicTabs" :key="index" :label="item" :value="item" />
					</el-select>
				</template>
			</table-title>
			...
			>
				<div v-for="(item, index) in RealtimeDataList" :key="index" class="realtime-scroll-item">
					<div class="category-name"><i class="icon-diamond" />{{ item.category }}</div>
...
import { getTotalStatistic, getRealtimeData, getBasicData } from "@/api/micro-service/data-management/";
