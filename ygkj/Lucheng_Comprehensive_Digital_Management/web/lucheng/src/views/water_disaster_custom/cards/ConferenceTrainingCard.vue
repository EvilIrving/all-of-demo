 ...
 * @LastEditors: ysl
 * @LastEditTime: 2021-09-14 10:48:17
 * @Description: 会议培训
 * @FilePath: /synthesizeSystem/src/views/water_disaster_custom/cards/conferenceTrainingCard.vue
-->
<template>
  <div class="conference-training-card">
    <panel-item
      title="会议培训"
      has-switch
      tc
      @tclick="tclick('会议培训')"
      :action-tabs="['总览', '培训次数', '报销金额', '支出类型']"
    ...
    >
      ...
      <template #title-switch>
        ...
        <i
          ...
          @click.stop="openDetailsDialog('会议培训')"
      ...
      <div class="is-water-box" v-if="activeTab == 2">
        ...
        <div id="oaAreaPie" style="width:50%;height:100%" ref="oaAreaPie"></div>
					...
					>
						...
						<span class="num">{{ item.value }}<span style="color:#fff;font-size:small">万元</span></span>
					</div>
      	</div>
      </div>
    </panel-item>
...
export default {
  ...
  data() {
    return {
      ...
      barData: {
        type: [
          { label: "预计伙食支出", value: 0 },
          { label: "预计住宿支出", value: 0 },
          { label: "预计其他费用", value: 0 },
        ],
      },
			totalLength: 0,
      barOption: {
  ...
  mounted() {
		this.getOaOverView()
    this.getMeetOverViewComponent()
    this.meetTrainCStati()
  },
  methods: {
    ...mapActions(["SetDetailDialogParam", "SetconferenceTrainingCount"]),
    // 设置C位数据
    async meetTrainCStati() {
      await oaApi.meetTrainCStati().then((res) => {
        // 获取c位数据
          this.SetconferenceTrainingCount({
            numberTrain: res.data.numberTrain,
            expenseAccountNumber: res.data.expenseAccountNumber,
            reimbursementAmount: res.data.reimbursementAmount
          });
      })
    },
    // 获取会议培训数据
    async getOaOverView() {
      ...
      let str = this.activeTab == 1 ? "meetTrainNumber" : this.activeTab == 2 ? "meetTrainReimbursementAmount" : "meetTrainSpendType"
      await oaApi[str](opt).then((res) => {
				this.activeTab == 1 || this.activeTab == 2 ? this[`barData${this.activeTab}`].rows = res.data
				: ""
				if( this.activeTab == 3 ) {
					this.barData.type[0] = res.data.mealsCost
          this.barData.type[1] = res.data.hotelCost
          this.barData.type[2] = res.data.otherCost
          this.totalLength = res.data.mealsCost + res.data.hotelCost + res.data.otherCost
				} 
      });
    ...
    async getMeetOverViewComponent() {
      let opt = {
        month: Number(this.checkMonth.split(' ')[0]),
      };
      this.rankingList = []
      await oaApi.meetOverViewComponent(opt).then((res) => {
        let data = res.data
        this.totalCount = data.meetTotalNum
        for(let item of data.list){
          this.rankingList.push({
            label: `${item.moon}月培训总数`, 
            total: item.total, 
            list: item.list
          })
        }
    ...
    openDetailsDialog(val) {
			this.$emit("openDialog", "会议培训")
    },
    // 标题点击打开统计卡片
    tclick(title) {
      this.$emit("tclick", "会议培训");
    },
    handleTabChange(val) {
      this.activeTab = val;
			this.barOption.setting.yAxisName[0] = this.activeTab == 1 ? "次" : "万元"
      this.activeTab == 0 ? this.getMeetOverViewComponent() : this.getOaOverView();
    },
    barConfig(options) {
...
<style lang="scss">
.conference-training-card .panel-item .panel-item__content--tabs .tab-button,
.conference-training-card .panel-item .panel-item__title--tabs .tab-button {
  display: inline-block;
  font-size: 16px;
  padding: 0 12px;
}
.conference-training-card .panel-item .panel-item__content--tabs .tab-button.active,
.conference-training-card .panel-item .panel-item__title--tabs .tab-button.active {
  padding: 0 12px;
}
</style>
