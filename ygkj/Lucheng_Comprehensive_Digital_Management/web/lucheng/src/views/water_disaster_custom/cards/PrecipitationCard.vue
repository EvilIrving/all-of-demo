  ...
  <div class="stagnant-water-card">
    <panel-item title="降水量" has-switch tc @tclick="tclick('降水量')">
      ...
      <template #title-switch>
        <RadioDropdown v-model="checkYear" :options="yearOption" @change="handleYear" />
        <i
          title="等值面"
          style="height: 18px"
        ...
        <i
          ...
          @click.stop="openDetailsDialog('降水量')"
...
export default {
  ...
  methods: {
      ...
      options.tooltip = {
        ...
        formatter: (val) => {
          let htmlText =
            ...
            `<p class="rivertooltip__item rivertooltip__item-error"><span>年平均降雨量: </span><span>${avg[
              val.dataIndex
            ].toFixed(2)}mm</span></p>` +
            `<p class="rivertooltip__item rivertooltip__item-error"><span>降水量: </span><span>${val.value}mm</span></p>` +
            `<p class="rivertooltip__item rivertooltip__item-warn"><span>占比: </span><span>${
              pre[val.dataIndex]
            }%</span></p>`;
          return htmlText;
        }
    ...
    showAreaPolygon() {
      ...
      let arr = this.barData1.rows.map((item) => {
        return item.data;
      });
      ...
      this.$emit('legend-change', '降水量公报', {
        ...
        children: [
          {
            label: `${parseFloat(onePercent * 4).toFixed(0)} - ${parseFloat(onePercent * 5).toFixed(0)}`,
            icon: 'evaluate-type5'
          },
          {
            label: `${parseFloat(onePercent * 3).toFixed(0)} - ${parseFloat(onePercent * 4).toFixed(0)}`,
            icon: 'evaluate-type4'
          },
          {
            label: `${parseFloat(onePercent * 2).toFixed(0)} - ${parseFloat(onePercent * 3).toFixed(0)}`,
            icon: 'evaluate-type3'
          },
          {
            label: `${parseFloat(onePercent).toFixed(0)} - ${parseFloat(onePercent * 2).toFixed(0)}`,
            icon: 'evaluate-type2'
          },
