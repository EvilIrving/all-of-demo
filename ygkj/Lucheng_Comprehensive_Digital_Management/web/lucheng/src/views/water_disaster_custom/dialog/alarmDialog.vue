    ...
    >
      <div class="primary-dialog__title" slot="title" :style="minimizeStatus ? 'justify-content: space-between;' : ''">
        <p class="primary-dialog__title--label" style="color: #fff">
          {{ mainTitle == '报警' || '山洪预警' ? '报警列表' : '水量平衡' }}
        </p>
      </div>
      <div v-if="mainTitle == '报警'" class="primary-dialog__content" style="height: 34rem">
        <div style="padding: 10px 20px; height: 50px">
          <span
            :class="num == i ? 'switchover-sty' : 'type-sty'"
            v-for="(item, i) in alarmList"
            :key="i"
            @click="switchData(i, item)"
            >{{ item.label }}({{ statsData[item.value] }})</span
          >
        ...
        <div style="padding: 10px 20px">
          <el-table ref="multipleTable" :data="realTimeData" height="450px" stripe width="100%">
            ...
            <!-- ①海塘报警表/海塘预警表（序号、时间、海塘名称、水位、蓝色警戒水位、黄色警戒水位、橙色警戒水位、红色警戒水位）；
            ②水位报警表（序号、时间、站点类型、名称、水位、警戒水位/汛限水位）+筛选项（站点类型——水库、河道、堰闸、潮位）
            ③降雨报警表（序号、时间、降雨报警类型、名称、降雨量）+筛选项（降雨报警类型——1小时超30mm、3小时超60mm）
            ④运行报警表（序号、时间、海塘名称、报警内容） ⑤台风报警表（序号、时间、台风名称、实时风力，距离鹿城） -->
            <!-- 1小时降雨  ，三小时降雨 -->
            <el-table-column
              v-if="num == '0' || num == '1'"
      ...
      <!-- 山洪预警 -->
      <div v-if="mainTitle == '山洪预警'" class="primary-dialog__content" style="height: 34rem">
        <FloodDialog></FloodDialog>
      </div>

      <!-- 用水量，供水量  -->
      <div v-else class="primary-dialog__content" style="height: 34rem">
        <div style="margin: 0 20px; display: flex; justify-content: flex-start; border-bottom: 1px solid #328173">
          <StatisticsBox title="总取水量" icon="icon-Water-withdrawal-icon">
            <span>{{ statisticsData.supply || '-' }} </span> 亿m³
          </StatisticsBox>
          <StatisticsBox title="总用水量" icon="icon-Water-withdrawal-icon">
            <span>{{ statisticsData.consume || '-' }} </span> 亿m³
          </StatisticsBox>
        </div>
        <div style="padding: 10px 20px">
        ...
        <el-row :gutter="20" style="padding: 0 20px">
          <el-col :span="12">
            <p class="title-sty">用水量报表</p>
            <el-table ref="multipleTable" :data="statisticsList.supplyList" height="350px" stripe width="100%">
              <el-table-column prop="index" label="序号" width="80">
                <template slot-scope="scope">
                  <span class="table-order">{{ scope.$index + 1 }}</span>
                </template>
              </el-table-column>
              <el-table-column :show-overflow-tooltip="true" prop="classifyName" label="用水量名称" />
              <el-table-column :show-overflow-tooltip="true" prop="year" label="年份" />
              <el-table-column :show-overflow-tooltip="true" prop="waterVolume" label="用水量" />
              <el-table-column :show-overflow-tooltip="true" prop="lastYearWaterVolume" label="上一年用水量" />
              <el-table-column label="变化">
                <template slot-scope="scope">
                  <i v-if="scope.row.compareLastYear == '1'" class="icon-on-the-change" />
                  <span v-else-if="scope.row.compareLastYear == '0'">-</span>
                  <i v-else class="icon-under-the-change" />
                </template>
              </el-table-column>
            </el-table>
          </el-col>
          <el-col :span="12">
            <p class="title-sty">取水量报表</p>
            <el-table ref="multipleTable" :data="statisticsList.consumeList" height="350px" stripe width="100%">
              <el-table-column prop="index" label="序号" width="80">
                <template slot-scope="scope">
                  <span class="table-order">{{ scope.$index + 1 }}</span>
                </template>
              </el-table-column>
              <el-table-column :show-overflow-tooltip="true" prop="classifyName" label="供水量名称" />
              <el-table-column :show-overflow-tooltip="true" prop="year" label="年份" />
              <el-table-column :show-overflow-tooltip="true" prop="waterVolume" label="取水量" />
              <el-table-column :show-overflow-tooltip="true" prop="lastYearWaterVolume" label="上一年用水量" />
              <el-table-column label="变化">
                <template slot-scope="scope">
                  <i v-if="scope.row.compareLastYear == '1'" class="icon-on-the-change" />
...
import FloodDialog from './count_dialog/FloodDialog.vue';
export default {
  name: 'PrimaryDialog',
  components: { StatisticsBox, LabelFormItem, FloodDialog },
  props: {
    visible: {
      type: Boolean,
      default: false
  ...
  methods: {
    // 取用水量接口
    waterResource() {
      this.watSupplyConsumeCompareList();
      this.stats4WatSupplyConsumeCompareList();
    },
    // 供水量-用水量年度对比表-表头统计
    async stats4WatSupplyConsumeCompareList() {
      await disasterPreventionApi.stats4WatSupplyConsumeCompareList({ year: this.year }).then((res) => {
        if (res.code == 0) {
    ...
    // 供水量-用水量年度对比表
    async watSupplyConsumeCompareList() {
      await disasterPreventionApi.watSupplyConsumeCompareList({ year: this.year }).then((res) => {
        if (res.code == 0) {
...
::v-deep.label-form-item .label-form-item__Label {
  color: #fff;
}
</style>
