 ...
 * @Description: 
 * @LastEditors: dtb
 * @LastEditTime: 2022-05-23 14:02:03
 * @FilePath: \lucheng\src\views\water_disaster_custom\dialog\SluiceDialogForSchedule\components\ReservoirIndex.vue
-->
<template>
  <div class="seawall">
    <div class="basic-info flex-box">
      <div>
        ...
        <div class="basic-info-box">
          <ReservoirInfo :propList="bascList" />
        </div>
      </div>
      <div class="flex-box">
        <div class="duty">
      ...
      <div class="alarm">
        ...
        <div class="alarm-box flex-box">
          <div class="alarm-box-one">
            <div class="alarm-box-title">水库超警</div>
            <div class="alarm-box-04">
              <div>
                <span>水库超警</span>
                <span
                  class="no-alarm-span"
                  :class="{ 'red-alarm-span': waterLevelWarnType.warning }"
                  >{{ !waterLevelWarnType.warning ? '正常' : '超警' }}</span
                >
              </div>
            </div>
          </div>
...
const displayPropsMap = {
  basc: [
    ...
    { name: '总库容(万㎡)', prop: 'totCap' }
  ],
  info: [
    { name: '坝型', prop: 'damType' },
...
export default {
  ...
  methods: {
    ...
    getProjectInfo() {
      hydraulicApi
        .getProjects({ withWaterLevel: true, type: '水库', code: this.pointData.projectCode || '' })
        .then((res) => {
          let resProp = res.data[0];
          this.bascList = [];
          this.infoList = [];
          for (let item of displayPropsMap['basc']) {
            this.bascList.push({ name: item.name, prop: item.prop, data: (resProp && resProp[item.prop]) || '-' });
          }
          for (let item of displayPropsMap['info']) {
            this.infoList.push({ name: item.name, prop: item.prop, data: (resProp && resProp[item.prop]) || '-' });
          }
        });
    },
    async getTyphoonDif() {
