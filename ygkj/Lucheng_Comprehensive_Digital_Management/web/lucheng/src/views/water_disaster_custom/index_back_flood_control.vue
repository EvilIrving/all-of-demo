<!--
 * @Date: 2022-05-11 15:13:26
 * @Author: 
 * @Description: 
 * @LastEditors: dtb
 * @LastEditTime: 2022-05-23 11:24:27
 * @FilePath: \lucheng\src\views\water_disaster_custom\index_back_flood_control.vue
-->
<script>
...
import ScreenFloodMap from './components/ScreenFloodMap.vue';
...
export default {
  ...
  methods: {
    ...
    renderLegend() {
      ...
      return (
        <div
          ...
          style={{ right: '30.5rem', bottom: this.RangeBar ? '18%' : '12%' }}
        >
     ...
     */
    showDialog(val) {
      console.log(val, 'flood 图层点击事件');
      this.cardInitFlag = false;
      let dialogList = ['抗旱监管', '防汛风险', '洪水风险图', '山洪','海塘'];
  ...
  render() {
    ...
    const { renderMap, renderLegend, renderStatCard, renderFactoryCard } = this;
    ...
    return (
      <div
        ...
        ref="bigScreen"
      >
        {[renderMap(), renderLegend(), renderStatCard(), renderFactoryCard()]}
        {addDialog}
        {saveAsDialog}
        {detailDialog}
        {WaterIntakeDialog}
