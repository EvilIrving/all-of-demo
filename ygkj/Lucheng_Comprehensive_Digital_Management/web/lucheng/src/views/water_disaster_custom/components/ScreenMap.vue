...
export default {
  name: 'ScreenMap',
  components: { ScreenFooter, PlayableTimelineBar, DistributionMap },
  props: {},
  data() {
    return {
      //地图
      ...
      selectClick.on('select', (e) => {
        ...
        if (features.length > 0) {
          ...
          if (projectTypeArr.includes(type)) {
            this.$emit('showProjectDialog', featureProperties.data, featureProperties);
    ...
    addWeatherWarnOverlay(list, type) {
      for (let i = 0; i < list.length; i++) {
        ...
        this.$nextTick(() => {
          ...
          console.log(overlay, 'overlayoverlayoverlayoverlayoverlay');
          this.map.addOverlay(overlay);
        });
      }
    },
