<!--
 * @Date: 2022-06-09 11:09:32
 * @Author: 
 * @Description: 灾害防御调度地图
 * @LastEditors: dtb
 * @LastEditTime: 2022-06-13 11:05:11
 * @FilePath: \lucheng\src\views\water_disaster_custom\components\ScreenBackMap.vue
-->
<template>
  <div class="map-wrap">
    ...
    />
    <!-- 水闸分布图 -->
    <distribution-map v-if="sluiceDistributionMap" />
    ...
    <!-- 点位悬浮框 -->
    <div v-show="popupWxqyShow" id="wxqyPopup" class="ol-popup">
      <!-- , isBigScrren && 'is-big-screen' -->
      <div :class="['ol-popup__wrapper']">
        <div class="ol-popup__title">{{ featureWxquInfo.name }}</div>
      </div>
    </div>
    ...
    >
      <div class="new_box">
        ...
        <div class="right">
          ...
          <!-- <div class="duty-box flex-box"></div> -->
...
import $ from 'jquery';
import DistributionMap from '../../WatershedSchedule/DistributionMap.vue';
import {
  drawClusterPoint,
  drawPoint,
  drawLines,
...
export default {
  name: 'ScreenBackMap',
  components: { ScreenFooter, PlayableTimelineBar, DistributionMap, PrimaryDialog },
  props: {},
  data() {
    return {
      //地图
  ...
  methods: {
    ...
    mapInit() {
      ...
      //   id: 'SLTCLayer',
      //   source: new XYZ({
      //     url: mapConfig.mapServerThree + 'arcgis/rest/services/lucheng/lc_weixiequyu/MapServer/tile/{z}/{y}/{x}',
    ...
    addWxqyOverlay() {
      this.overlay4 = new Overlay({
        //设置弹出框的容器
        element: document.querySelector('#wxqyPopup'),
        autoPan: true,
        autoPanAnimation: {
          duration: 250
        }
      });
      this.map.addOverlay(this.overlay4);
    },
    addWaterSourOverlay() {
    ...
    addWaterChangeFixedOverlay(list, type, tab) {
      for (let i = 0; i < list.length; i++) {
        ...
        let idx = searchAreaList.indexOf(list[i].areaName);
        let coodinate = idx == -1 ? [] : centerPoint[idx];
        $('#MainViewSche').append(tab == '水域面积' ? domText1 : domText2);
        this.$nextTick(() => {
          let overlay = new Overlay({
            id: type + i,
            element: document.getElementById(type + i),
    ...
    registeredEvent() {
      ...
      this.map.on('pointermove', (event) => {
        ...
        this.map.forEachFeatureAtPixel(pixel, (feature) => {
          ...
          // console.log(feature.values_,feature.getProperties());
          ...
          } else if (feature && feature.values_.type == 'wxqy') {
            // console.log(properties);
            this.featureWxquInfo = {
              name: properties.NVI
              // props: properties.features[0].values_.data.props,
              // label: properties.features[0].values_.data.label || false
            };
            if (this.featureWxquInfo) {
              this.overlay4.setPosition(coodinate);
              this.popupWxqyShow = true;
            }
          } else {
    ...
    //加载鹿城威胁区域
    addLcwxqyGeoJsonLayer(lcwxqyGeoJson) {
      let vectorSource = new VectorSource({
        features: new GeoJSON().readFeatures(lcwxqyGeoJson)
      });
      // console.log(vectorSource.getFeatures(),'lcwxqyGeoJson');
      if (this.threatAreaLayers != null) {
        this.map.removeLayer(this.threatAreaLayers);
        this.threatAreaLayers = null;
      }
      this.threatAreaLayers = new VectorLayer({
        source: vectorSource,
        // opacity: 0.85,
        style: (feature) => {
          feature.values_.type = 'wxqy';
          if (feature.values_.LVL != '0') {
            const style = new Style({
              fill: new Fill({
                color: 'rgba(241,0,0,0.2)'
              }),
              stroke: new Stroke({
      ...
      this.map.addLayer(this.threatAreaLayers);
    },
    ...
    drawDynamicPoint(coordinates, typeName) {
      ...
      let mapDom = document.getElementById('MainViewSche');
    ...
    inputChange() {
      ...
      if (this.templateTypeValue === 0) {
        // console.log(this.content.substring(this.content.length-1,this.content.length));
...
.treeStyle {
  ...
  ::v-deep .el-tree-node__content:hover {
    background: linear-gradient(90deg, rgba(0, 196, 255, 0.3), rgba(41, 147, 180, 0.1));
    color: #00baff;
  }
