<!--
 * @Date: 2022-05-06 16:17:53
 * @Author: 
 * @Description: 
 * @LastEditors: dtb
 * @LastEditTime: 2022-05-06 16:17:54
 * @FilePath: \lucheng\src\components\panel_item\PanelItem.vue
-->
<template>
  <div class="panel-item">
    <div v-if="title" class="panel-item__title drag">
      <!-- <div class="panel-item__title--light" /> -->
      <div v-if="title.icon" class="panel-item__title--icon">
        <i :class="title.icon" />
      </div>
      <div class="panel-item__title--label">
        <span class="label-text" @click="handleTitleClick(title)" :style="{ cursor: to || tc ? 'pointer' : 'unset' }">
          <slot name="title-label">{{ title || title.text }}</slot>
        </span>
        <span v-if="seawallRangModule && title=='海塘'" class="in-simulation">演练中</span>
      ...
      <div v-if="dropdown" class="panel-item__title--select">
        <el-select v-model="selected" placeholder="请选择">
          <el-option v-for="(item, index) in dropdown" :key="index" :label="item" :value="item" />
        </el-select>
      </div>
      <div v-if="hasSwitch || doubleSwitch" class="panel-item__title--switch">
        <slot name="title-switch">
          ...
          <el-switch v-if="doubleSwitch" v-model="value2" @change="handleSwitchChange(2)" />
        </slot>
        <i v-if="title != '水雨情报警' && title != '台风报警'" class="icon-chose" @click.stop="closeCard"></i>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['StatisticFlag', 'seawallRangModule'])
  },
  data() {
    return {
      activeTab: 0,
  ...
  methods: {
    ...
    ...mapActions(['SetActiveCount', 'SetStatisticFlag', 'changeSeawallRangModule']),
    /**
     * 处理tab切换并将事件冒泡
     */
      ...
      if (this.tc) {
        switch (this.title) {
          ...
          case '山洪':
            ...
            break;
          default: {
            this.SetActiveCount(this.title);
          }
        }
        !this.StatisticFlag && this.SetStatisticFlag(true);
      } else {
        this.$emit('tclick');
      }
    }
