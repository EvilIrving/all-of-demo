    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-else>
            <span v-if="item.lessZeroEmpty">{{ scope.row[item.prop] < 0 ? ' - ' : scope.row[item.prop] }}</span>
            <span v-else>{{ scope.row[item.prop] || scope.row[item.prop] == 0 ? scope.row[item.prop] : '0' }}</span>
          </div>
        </template>
      </el-table-column>
    </el-table>
