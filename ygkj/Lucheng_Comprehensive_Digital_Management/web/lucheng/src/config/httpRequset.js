/*
 * @Author: lsh
 * @Date: 2021-01-18 09:47:03
 * @LastEditors: Please set LastEditors
 * @LastEditTime: 2022-10-14 14:48:42
 * @FilePath: \lucheng\src\config\httpRequset.js
 */
...
service.interceptors.response.use(
  (response) => {
    ...
    if (res.code !== 0) {
      ...
      if (res.code === 401) {
        ...
        window.location.href = 'https://slgh.lucheng.gov.cn/lucheng/login'
      }
  ...
  (err) => {
    ...
    if (new Error(err) != 'Error: Error: Network Error' && err.response == undefined) {
    ...
    if (err.response != undefined && err.response.status) {
      ...
      return Promise.reject(new Error('请求失败! 状态码 ' + err.response.status));
    }
  }
);
