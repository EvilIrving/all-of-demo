 ...
 *
 * @typedef {Window&globalThis&global} extendWindow
 */
/** @type {extendWindow} */
 ...
 * @property {[number,number]} [anchor] 锚点位置
 * @property {import('ol/style/IconAnchorUnits').default} [anchorXUnits] X轴锚点单位
 * @property {import('ol/style/IconAnchorUnits').default} [anchorYUnits] Y轴锚点单位
 * @property {string} [src] 图片路径
 * @property {string} [title] 标题
 * @property {number} [textOffsetX] 文字X轴偏移
 * @property {number} [textOffsetY] 文字Y轴偏移
 * @property {number} [scale] 文字缩放
 */

/**
 * 聚合点位样式类型
 * @typedef {object} ClusterPointOption
 * @property {number} [distance] 聚合间距
 * @property {string} [srcs] 聚合点位图片路径
 *
 * @typedef {ClusterPointOption&PointStyle} ClusterPointStyle
 */
...
export function drawPoint(coordinates, styleObj) {
	...
	let vectorLayer = new Vector({
		...
		zIndex: 100,
		style: feature => {
			const mapZoom = extendWindow.mapZoom || 1;
			let style;
			let titleText;
			...
			if (mapZoom > 11) {
				...
				if (featureData.modular != "流域灾害风险评估") {
					titleText = new Text({
...
export function drawLine(coordinates, styleObj) {
	...
	const vectorLayer = new Vector({
		...
		style: new Style({
			fill: new Fill({
				color: styleObj.lineColor || "rgba(255, 255, 255, 0.1)"
			}),
			stroke: new Stroke({
...
/**
 * 绘制带两端点位的线段
 * @param {any[]} coordinatesList [[[lng,lat]],[lng,lat]...]
 * @param {LineStyle} styleObj {}
 * @returns
 */
export function drawLineEx(coordinatesList, styleObj, layerName) {
	let vectorSource = new VectorSource();
	const vectorLayer = new Vector({
		source: vectorSource,
		style: new Style({
    ...
    let obj = { geometry: plygon, data: coordinates.data }
    if (layerName) {
...
export function drawPolygon(coordinates, styleObj, polygonType) {
	...
	const vectorPolygonLayer = new Vector({
		...
		style: new Style({
			fill: new Fill({
				color: styleObj.fillColor || "rgba(124,222,0, 0.1)"
			}),
			stroke: new Stroke({
...
export function drawLineInMeter(arr, radius) {
	var linestring1 = turf.lineString(arr, { name: 'line 1' });
	var buffered = turf.buffer(linestring1, radius, {});

	let source = new GeoJSON().readFeatures(buffered, {
...
const stationTypeArr = ['水库水位', '断面', '河道水位', '堰闸水位', '潮位', '闸门开度', '水质监测', '积水'];
// const stationTypeArr = ['水库', '山塘', '堤防', '海塘', '泵站', '水闸', '闸站', '灌区', '农饮水', '水电站'];
//周边分析点位
export function drawAnaylysiscetPoint(coordinates) {
	let vectorSource = new VectorSource({});
	for (let i = 0; i < coordinates.length; i++) {
		const type = coordinates[i].analysisType;
		let imgSrc = coordinates[i].src;
		const longitude = coordinates[i].longitude;
		const latitude = coordinates[i].latitude;
		if (!latitude || !longitude || !imgSrc) continue;
		if (stationTypeArr.indexOf(type) != -1) {
...
/**
 * 画点方法（聚合）
 * @param {any[]} coordinates 点经纬度数组
 * @param {ClusterPointStyle} styleObj 点位样式
 * @returns
 */
export function drawClusterPoint(coordinates, styleObj) {
	/**
	 * @type {Feature<import("ol/geom").Geometry>[]}
	 */
	 ...
	 * 聚合源
	 */
	const clusterSource = new Cluster({ distance: styleObj.distance || 20, source: new VectorSource({ features }) });

	/**
	 * 计算聚合点位样式
	 * @type {import("ol/style/Style").StyleFunction} feature
	 */
	const computedClusterStyle = feature => {
		const mapZoom = extendWindow.mapZoom || 1;
		const size = feature.get("features").length;
		/** @type {import('ol/Style/Text').Options} */
		let textOption = { font: "500 14px 微软雅黑", textAlign: 'center', padding: [1, 2, 1, 2] };
		...
		if (size == 1 && mapZoom > 12) {
			...
			textOption = {
				...
				offsetX: styleObj.textOffsetX || 13,
		...
		return new Style({
			image: new Icon({
				...
				// @ts-ignore
				anchorYUnits: styleObj.anchorYUnits || "pixels",
				src: size == 1 ? featureData.src || styleObj.src : featureData.srcs || styleObj.srcs || "",
				scale: styleObj.scale || 1
			}),
			text: titleText
		});
...
export function showGeneralizaPoint(pointData) {
	...
	let vectorLayer = new Vector({ source: vectorSource });
	return vectorLayer;
}
