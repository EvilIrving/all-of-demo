<template>
  <div class="project-wrap">
    <van-nav-bar fixed placeholder safe-area-inset-top :title="'详情'"  :left-arrow="$store.state.viewType !== 'irs'" @click-left="$router.go(-1)" />
    <van-cell-group>
      <van-cell
          ...
          >
            <el-card class="card-box">
              ...
              <p>{{ item.status | statusType }}状态：
                <span class="orange-color" @click="toSuggestion(item)">{{
                    item.status | status
                  }}</span>
                <span v-if="item.status===1&&item.notice!==1" class="card-btn" @click="setMsgRead(item)">提醒</span>
                <span v-if="item.notice===1" class="card-btn" @click="setMsgRead(item)">已提醒</span>
              </p>
