  ...
  <view-page>
    <div class="new_task">
      <van-cell-group>
        ...
        />
        <van-field
          v-model="form.hiddenDanger"
          label="具体隐患"
          placeholder="请输入"
        />
        <van-field is-link readonly label="隐患图片(视频)">
          <template #input>
            <el-upload
...
export default {
  ...
  methods: {
    ...
    handlePicture(res, file) {
      if (res.code == 0) {
        let index = this.picList.findIndex((item) => item.uid == file.uid);
        this.picList[index] = {
          ...this.picList[index],
          ...res.data,
        ...
        let index = this.picList.findIndex((item) => item.uid == file.uid);
        this.$message.error(`${this.picList[index].name}上传失败，请重新上传`);
