<template>
  <view-page>
    <div class="progress_detail">
        ...
        <div class="block">
          <el-timeline>
            ...
            <el-timeline-item
              ...
              :color="form.handlingStatus == 2 ? '#036EFF' : ''"
              placement="top"
            >
              <el-card class="card-box">
                ...
                <p>
                  ...
                  <span
                    :class="{
                      'blue-color': form.handlingStatus == 2,
                      'orange-color': form.handlingStatus == 1,
                    }"
                    >{{ form.handlingStatus == 1 ? "待处理" : "已处理" }}</span
                  >
                </p>
              </el-card>
            </el-timeline-item>
      ...
      <van-button
        type="danger"
        v-if="form.archivingStatus == 1"
        block
        @click="linkTask"
        >{{ form.handlingStatus == 1 ? "处理" : "归档" }}</van-button
      >
      <van-popup
        v-model="showPopup"
        position="bottom"
...
export default {
  ...
  methods: {
    ...
    linkTask() {
      if (this.form.handlingStatus == 1) {
        this.$router.push(
          "/propertySupervision/disposeInfo?id=" + this.form.id
        );
      } else {
        this.onConfirm();
      }
    ...
    async onConfirm() {
      let res = await handingInspectionApi({ id: this.form.id, handType: 3 });
      if (res.code == 0) {
        this.$toast.success("已归档成功~");
        this.getList();
      } else {
