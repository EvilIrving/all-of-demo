...
public class JwtTokenUtil {
    ...
    public static void setTokenSettings(TokenSettings tokenSettings) {
        ...
        accessTokenExpireTime = tokenSettings.getAccessTokenExpireTime() == null ?
                Duration.ofHours(8) : tokenSettings.getAccessTokenExpireTime();
    ...
    public static List<String> getRoleList(String token) {
        ...
        try {
            Claims claims = getClaimsFromToken(token);
            roleList = (List<String>) claims.get(Constants.JWT_ROLES_KEY);
        } catch (Exception e) {
            log.error("error={}", e);
        }
        return roleList;
    ...
    public static boolean external(String token) {
        boolean external = false;
        try {
            external = getClaimsFromToken(token).get(Constants.EXTERNAL_KEY, Boolean.class);
        } catch (Exception e) {
            log.error("error={}", e);
        }
        return external;
    ...
    public static Optional<String> getCurrentUserId() {
        ...
        if (!StringUtils.isEmpty(token)) {
            ...
            // 李昶的id，此处不要提交，为了测试
            userId = "61fe30e6ffb64e7288d9f2cdb79e6658";
        }
