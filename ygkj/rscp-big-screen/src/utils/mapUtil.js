/*
 * @Author: wupengfei
 * @LastEditTime: 2022-10-17 20:18:29
 * @LastEditors: Please set LastEditors
 * @Descripttion:
 * @FilePath: \rscp-big-screen\src\utils\mapUtil.js
 */
...
// 水库智管画点方法
export function skzgDrawPoint(coordinates) {
  let vectorSource = new VectorSource({});
  coordinates.forEach((element) => {
    ...
    let style = {
      image: new Icon({
        anchor: element.styleObj.anchor || [1, 1],
...
export function drawPoint(coordinates, styleObj, showName, type) {
  console.log(showName, type, 'type');
  let vectorSource = new VectorSource({});
  coordinates.forEach((element) => {
    ...
    if (type == 'dike') {
      src = require("@/assets/images/map/basin-dike.png");
    } else if (type == 'sluice') {
      src = require("@/assets/images/map/basin-sluice.png");
    } else if (type == 'pump') {
      src = require("@/assets/images/map/basin-pump.png");
    } else if (type == 'gate') {
      src = require("@/assets/images/map/basin-gate.png");
    } else if (type == 'tide') {
      src = require("@/assets/images/map/tide-njh.png");
    } else if (type == 'cwyj') {
      src = require("@/assets/images/map/gao_risk.png");
    }
    const iconFeature = new Feature({
...
export function drawPolygonByFeature(feature, styleObj) {
  let vectorPolygonSource = new VectorSource();
  const vectorPolygonLayer = new Vector({
    source: vectorPolygonSource,
    style: new Style({
...
export function drawGeoJson(data, styleObj) {
...
export function newTyphoon(map, typhoonData, typhoonLayerMap, vueComponent, noPlayTyphoon) {
  this.map = map;
  ...
  this.init = function () {
    ...
    this.map.addOverlay(vueComponent.lyr);
    imgElement.src = require("../assets/images/typhoon.gif");
    vueComponent.lyr.setPosition([this.typhoonData.points[this.typhoonData.points.length - 1].lng, this.typhoonData.points[this.typhoonData.points.length - 1].lat]);
  };
    ...
    this.map.on("click", (evt) => {
      ...
      if (this.map.hasFeatureAtPixel(pixel)) {
        ...
        if (attr) {
          ...
          vueComponent.lyr.setPosition([attr.lng, attr.lat]);
          const feature = new Feature({
            geometry: new Point(attr.end),
            attr: attr,
          });
          this.typhoonLayers
            .get(attr.tfbh + "layer")
  ...
  this.typhoonStyle = (feature) => {
    ...
    if (type === "live" && index === this.typhoonPlayIndex[attr.tfbh]) {
      ...
      if (attr.radius7) {
        ...
        //   },
        // 右上|右下|左上|左下
        let radiusArr = attr.radius7.split('|')
        let radius7_quad = {
          'se': radiusArr[1], //左下
          'sw': radiusArr[3], //右下
          'ne': radiusArr[0], //右上
        ...
        const geometry = getCircleGeometry(center, radius7_quad);
      ...
      //10级风圈样式
      if (attr.radius10) {
        let radiusArr = attr.radius10.split('|')
        let radius10_quad = {
          'se': radiusArr[1], //左下
          'sw': radiusArr[3], //右下
          'ne': radiusArr[0], //右上
        ...
        const geometry = getCircleGeometry(center, radius10_quad);
        const style = new Style({
          geometry: geometry,
          stroke: new Stroke({
            color: "#eb7b7c",
      ...
      //12级风圈样式
      if (attr.radius12) {
        let radiusArr = attr.radius12.split('|')
        let radius12_quad = {
          'se': radiusArr[1], //左下
          'sw': radiusArr[3], //右下
          'ne': radiusArr[0], //右上
        ...
        const geometry = getCircleGeometry(center, radius12_quad);
        const style = new Style({
          geometry: geometry,
          stroke: new Stroke({
            color: "#eb7b7c",
