<!--
 * @Date: 2022-10-15 20:48:57
 * @Author: 
 * @Description: 
 * @LastEditors: dtb
 * @LastEditTime: 2023-02-15 20:23:54
 * @FilePath: \rscp-big-screen\src\views\IntelligenceScreen\blueSeawall\components\ProjectPopup.vue
-->
<template>
  <div class="">
    <div class="proj_content">
      <div class="dialog_title">
        <!-- 海塘信息 -->
        <span style="cursor: pointer" @click="lookDetail(pointData)">{{
          pointData.name
        }}</span>
        <span style="font-size: 16px" @click="lookDetail(pointData)">
          {{ pointData.scale }}</span>
      ...
      <div class="proj_top">
        ...
        <div class="proj_left">
          ...
          <div class="proj_title">
            <p>
              <span>{{ pointData.cityADNM }} - {{ pointData.countryADNM }}</span>
            </p>
          </div>
          <div class="proj_detail">
            ...
            <div class="popup_item">
              <p>海塘类型：</p>
              <span>{{
                pointData.seawall_ct_type ? pointData.seawall_ct_type : "-"
              }}</span>
            </div>
            <div class="popup_item">
              <p>海塘形式：</p>
              <span>{{
                pointData.seawall_pattern ? pointData.seawall_pattern : "-"
              }}</span>
            </div>
            <div class="popup_item">
              <p>海塘长度：</p>
              <span>{{
                pointData.seawall_length ? pointData.seawall_length + "m" : "-"
              }}</span>
            </div>
            <div class="popup_item">
              <p>安全管理状态：</p>
            ...
            <div class="popup_item">
              <p>设计潮位：</p>
              <span>{{ pointData.designTideLevel ? pointData.designTideLevel : "-" }}</span>
            </div>
            <div class="popup_item">
              <p>实时潮位：</p>
              <span>{{ pointData.realTimeTide ? pointData.realTimeTide : "-" }}</span>
            </div>
            <div class="popup_item">
              <p>更新时间：</p>
              <span>{{ pointData.realTime ? pointData.realTime.substring(10,16) : "-" }}</span>
            </div>
          </div>
        </div>
        <!-- 上部分-右侧 -->
    ...
    <div v-show="drawer" tabindex="0" @focus="focusDiv" @blur="blurDiv" outline="none" hidefocus="false" class="drawer">
      ...
      <ul>
        ...
        <h4>安全鉴定</h4>
        <li v-for="item in reasionList.slice(2, 6)" :key="item.reason">
          <img v-show="item.brighting === 'yellow'" :src="$config.imgUrl + 'huang.png'" alt="" />
          <img v-show="item.brighting === 'red'" :src="$config.imgUrl + 'cheng.png'" alt="" />
          <img v-show="item.brighting === 'off'" :src="$config.imgUrl + 'offpoint.png'" alt="" />
          {{ item.reason }}
        ...
        <h4>巡查检查</h4>
        <li v-for="item in reasionList.slice(6,8)" :key="item.reason">
          <img v-show="item.brighting === 'yellow'" :src="$config.imgUrl + 'huang.png'" alt="" />
          <img v-show="item.brighting === 'red'" :src="$config.imgUrl + 'cheng.png'" alt="" />
          <img v-show="item.brighting === 'off'" :src="$config.imgUrl + 'offpoint.png'" alt="" />
          {{ item.reason }}
        ...
        <h4>缺口封堵</h4>
        <li v-for="item in reasionList.slice(8,9)" :key="item.reason">
          <img v-show="item.brighting === 'yellow'" :src="$config.imgUrl + 'huang.png'" alt="" />
          <img v-show="item.brighting === 'red'" :src="$config.imgUrl + 'cheng.png'" alt="" />
          <img v-show="item.brighting === 'off'" :src="$config.imgUrl + 'offpoint.png'" alt="" />
          {{ item.reason }}
        ...
        <h4>工程施工</h4>
        <li v-for="item in reasionList.slice(9, 10)" :key="item.reason">
          <img v-show="item.brighting === 'yellow'" :src="$config.imgUrl + 'huang.png'" alt="" />
          <img v-show="item.brighting === 'red'" :src="$config.imgUrl + 'cheng.png'" alt="" />
          <img v-show="item.brighting === 'off'" :src="$config.imgUrl + 'offpoint.png'" alt="" />
          {{ item.reason }}
...
export default {
  ...
  data() {
    return {
      ...
      reasionList: [
        // 预报潮位
        { brighting: "off", code: "R4", reason: "超设计潮位" },
        { brighting: "off", code: "R5", reason: "接近设计潮位30cm以内" },
        // 安全鉴定
        ...
        // 巡查检查
        { brighting: "off", code: "R7", reason: "存在严重问题" },
        { brighting: "off", code: "Y4", reason: "存在较严重问题" },
        // 缺口封堵
        { brighting: "off", code: "R5", reason: "未封堵" },
        // 工程施工
        { brighting: "off", code: "Y5", reason: "未停工" },
  ...
  watch: {
    popupData(val) {
      if (val.mapType == 'seawallPopup') {
        this.pointData = val
        // this.tyCauseCode = val.tyCauseCode
        this.getPhotoDetailList(val.prcd, val.name);
        this.getPersonList(val.prcd);
        this.getNewPopupData()
        this.riskHIddenData(val.tyCauseCode)
      }
    },
  },
  mounted() { },
  methods: {
    ...
    // 三色码显示原因
    async riskHIddenData(tyCauseCode) {
      this.reasionList.forEach((i) => {
        i.brighting = 'off'
      })
      let tempArr = tyCauseCode ? tyCauseCode.split(',') : []
      if (tempArr.length) {
        tempArr.forEach((i) => {
    ...
    closePopup() {
      this.$parent.closeHtPopup();
    },
    // 弹窗内打开查看图片
    lookPhotos(row) {
      this.$parent.$parent.openLookPhotosPopup();
      this.$parent.$parent.showPhotosList(this.photoList, row.name);
    },

    // 弹窗内打开查看工程详情
    lookDetail(row) {
      this.$parent.$parent.openDetailPopup(row);
    },
