  ...
  <div class='popup_map'>
    ...
    <div class="dialog_body">
      <section class="work_left">
        <div class="right_content">
          <p class="third_title">
            ...
            <span style="font-size:16px">隐患清单</span>
            <i></i>
        ...
        <div ref="tb_wrappers" class="sk_table_wrapper" style="margin-top:10px">
          <el-table highlight-current-row border v-loading='loading' height='400px' :data="siteList" style="width: 100%;" @row-click="clickProject">
            ...
            <el-table-column label="级别" align="center" width="49px">
              <template slot-scope="scope">
                <span v-if="scope.row.level ==='一般'" class="yellow">一般</span>
                <span v-if="scope.row.level === '较重'" class="orange">较重</span>
                <span v-if="scope.row.level === '严重'" class="red">严重</span>
              </template>
            </el-table-column>
            <el-table-column :label="'结果'" align="center" width="49px">
              <template slot-scope="scope">
                <i v-if="scope.row.problemStatus === '已处置'" class="disposed"></i>
                <i v-if="scope.row.problemStatus === '处置中'" class="disposeing"></i>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <section class="work_right" v-loading='loading'>
        <p class="third_title blackstyle">
          <img class="point" :src="$config.imgUrl+ 'point.png'" alt="">
          <span style="font-size:16px">处置流程闭环图</span>
          <i></i>
          <img class="arr" :src="$config.imgUrl+ 'arr.gif'" alt="">
        </p>
        <!-- 流程闭环图 -->
        <div class="closed_loop_diagram">
          <ul>
            <li>
              ...
              <i class="lines"></i>
              <section>
                <p>上传人：{{infoDetail.psnm}}</p>
                <p>{{infoDetail.fxTm || '-'}}</p>
              </section>
              <div class="textone" :class="infoDetail.state > 1? '' : 'black'">上报后流转至处置人</div>
              <img v-if="infoDetail.state > 1" class="lingsone" :src="$config.imgUrl+ 'lujing01.png'" alt="">
              <img v-else class="lingsone" :src="$config.imgUrl+ 'lujing01-black.png'" alt="">
            </li>
            <li>
              <section>
                <i v-if="infoDetail.state > 1" class="handle"></i>
                <i v-else class="handle-black"></i>
                <span :class="infoDetail.state > 1 ? '' : 'black'">处置</span>
              </section>
              <i class="lines"></i>
              <section :class="infoDetail.state > 1 ? '' : 'black'">
                <p>处置人：{{infoDetail.zgname}}</p>
                <p>{{infoDetail.rectifyTm || '-'}}</p>
              </section>
              <div class="textone" :class="infoDetail.state > 2 ? '' : 'black'">处置后流转至审核人</div>
              <img v-if="infoDetail.state > 2" class="lingsone" :src="$config.imgUrl+ 'lujing01.png'" alt="">
              <img v-else class="lingsone" :src="$config.imgUrl+ 'lujing01-black.png'" alt="">
            </li>
            <li>
              <section>
                <i v-if="infoDetail.state > 2" class="handle"></i>
                <i v-else class="handle-black"></i>
                <span :class="infoDetail.state > 2 ? '' : 'black'">结束</span>
              </section>
              <i class="lines"></i>
              <section :class="infoDetail.state > 2 ? '' : 'black'">
                <p>整改人：{{infoDetail.zgname}}</p>
                <p>{{infoDetail.rectifyTm || '-'}}</p>
              </section>
            </li>
            <div class="texttwo" :class="infoDetail.state > 2 ? '' : 'black'">上传人可通过系统查看问题处置结果;</div>
            <div class="textthree" :class="infoDetail.state > 2 ? '' : 'black'">问题处置结束，结果将反馈至上传人</div>
            <img v-if="infoDetail.state > 2" class="lingstwo" :src="$config.imgUrl+ 'lujing02.png'" alt="">
            <img v-else class="lingstwo" :src="$config.imgUrl+ 'lujing02-black.png'" alt="">
          </ul>
        </div>
        <p class="third_title" style="margin:5px">
          <img class="point" :src="$config.imgUrl+ 'point.png'" alt="">
          <span style="font-size:16px">处置详情</span>
          <i></i>
          <img class="arr" :src="$config.imgUrl+ 'arr.gif'" alt="">
        </p>
        <div class="detail_box">
          <p style="padding:10px 10px 0px 10px">行政区划： <span>{{popupData.cityADNM}} - {{popupData.countryADNM}}</span></p>
          <el-timeline style="padding:10px ;height:calc(100% - 49px); overflow: auto;">
            <!-- 处置中 -->
            <el-timeline-item color="#00cfff" :timestamp="infoDetail.TM" placement="top" class="timeline-item">
              <div class="card_box">
                <p>{{ infoDetail.peoblemCont  || '-'}}</p>
                <!-- <div class="fileUrl">
                ...
                <div class="info">
                  <i class="person"></i> <span>{{ infoDetail.psnm }} </span>&nbsp;<span style="color:#ccc" v-show="infoDetail.psnm">|</span>&nbsp;<span> {{ infoDetail.phone }}</span>
                </div>
              </div>
            </el-timeline-item>
            <el-timeline-item v-show="infoDetail.state > 1" color="#00cfff" :timestamp="infoDetail.rectifyTm" placement="top" class="timeline-item">
              <div class="card_box">
                <p>{{ infoDetail.zgname || '-' }}</p>
                <!-- <div class="fileUrl">
                  <el-image v-show="infoDetail.RESPONSEPHOTOS"  :src="item.filurl" v-for="(item,index) in infoDetail.RESPONSEPHOTOS" :key="index">
                    <div slot="error" class="image-slot">
                      <i class="el-icon-picture-outline"></i>
                ...
                <div class="info">
                  <i class="person"></i> <span>{{ infoDetail.zgname }} </span>&nbsp;<span style="color:#ccc" v-show="infoDetail.zgname">|</span>&nbsp;
                </div>
              </div>
            </el-timeline-item>
            <!-- 四平台 -->
            ...
            <!-- 审核通过 -->
            <el-timeline-item v-show="infoDetail.state > 2" color="#00cfff" :timestamp="infoDetail.resTm" placement="top" class="timeline-item">
              <div class="card_box">
                <p>{{ infoDetail.resContent || "-" }}</p>
                <div class="info">
                  <i class="person"></i> <span>{{ infoDetail.resUser }} </span>&nbsp;<span style="color:#ccc" v-show="infoDetail.resUser">|</span>&nbsp;<span> {{ infoDetail.resUserPhone }}</span>
                </div>
              </div>
            </el-timeline-item>
          </el-timeline>
...
export default {
  ...
  methods: {
    clickProject(row) {
      let temp = row
      if (temp.problemStatus == '处置中') {
        temp.state = 2
      } else if (temp.problemStatus == '已处置') {
        temp.state = 3
      } else {
        temp.state = 1
      }
    ...
    async getProblemNum() {
      ...
      if (res.success) {
        this.tabList[0].num = res.data.problemNum
    ...
    async getProblemList() {
      ...
      if (res.success && res.data.length !== 0) {
        ...
        if (res.data[0].problemStatus == '处置中') {
          res.data[0].state = 2
        } else if (res.data[0].problemStatus == '已处置') {
          res.data[0].state = 3
        } else {
          res.data[0].state = 1
        }
