<template>
  <div class="popup_map">
    ...
    <div v-show="drawer" tabindex="0" @focus='focusDiv' @blur.native='blurDiv' outline="none" hidefocus="false" class="drawer">
      <p class="third_title">
    ...
    <img v-show="drawer" class="left_menu cursor" @click='closeDiv' :src="$config.imgUrl + 'left_colse_menu.png'" alt="">
  </div>
</template>
...
export default {
  ...
  watch: {
    popupData(val) {
      if (['seawallPopup', 'firstLinePopup', 'secondLinePopup'].includes(val.mapType)) {
        this.pointData = val
