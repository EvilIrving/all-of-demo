<template>
  <div class="popup_map">
    <div class="proj_content">
      <div class="dialog_title">
        ...
        <div class="title_img_box">
          <img class="title_img" @click="lookStatus(colorCode)" v-if="colorCode && colorCode == '红色'" :src="$config.imgUrl + 'status-red.png'" alt="">
          <img class="title_img" @click="lookStatus(colorCode)" v-if="colorCode && colorCode == '黄色'" :src="$config.imgUrl + 'status-yellow.png'" alt="">
          <img class="title_img" v-if="colorCode && colorCode == '绿色'" :src="$config.imgUrl + 'status-green.png'" alt="">
        </div>
    ...
    <primary-dialog mainTitle="发送消息" :visible="showDialog" @handleClose="showDialog = false" @changeFullscreen="changeFullscreen"
      :fullscreen="fullscreen"  width="500px" append-to-body top="30vh">
      <div class="dialog-content send_dialog">
        <!-- <div class="dialog-content send_dialog" v-if="isZzd"> -->
        <div>
    ...
    <div v-show="drawer" tabindex="0" @focus.stop='focusDiv' @blur.stop='blurDiv' outline="none" hidefocus="false" class="drawer">
      <p class="third_title">
    ...
    <img v-show="drawer" @click="blurDiv" class="left_menu cursor" :src="$config.imgUrl + 'left_colse_menu.png'" alt="">
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.getPhotoDetailList(this.popupData.PRCD, this.popupData.NAME);
    this.getVideoDetailList(this.popupData.PRCD);
    this.getCameraList(this.popupData.PRCD);
    this.getPersonList(this.popupData.PRCD);
    this.getNewPopupData()
    this.riskHIddenData(this.popupData.PRCD)
  },
  methods: {
    changeFullscreen() {
    ...
    async riskHIddenData(prcd) {
      ...
      if (res.success) {
        this.colorCode = res.data.threeYards
...
.popup_map {
  ...
  .drawer {
    ...
    right: 0%;
    bottom: 5%;
    background-color: rgba($color: #0431a0, $alpha: 0.5);
    color: #333;
    font-size: 16px;
    overflow-y: auto;
    outline: none;
