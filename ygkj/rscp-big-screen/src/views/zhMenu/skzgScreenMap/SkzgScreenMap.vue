<template>
  <div class="map_wrap">
    <div
      ref="myModal"
      @click="showDropDir = false"
      :style="{ display: showDropDir ? 'block ' : 'none' }"
      class="modal"
    ></div>
    <div class="map_back" v-show="showMapBack" @click="mapBack()">返回</div>
    <div class="map_switch">
      <div class="">
        <h2 :title="nowAdcdName || nowBasinName" class="adnm_switch">
          {{ nowAdcdName || nowBasinName }}
          <i
            v-show="showDropDir"
            @click.stop="showSwitchBtn(true)"
            class="dropdown cursor"
            key="dropdown"
          ></i>
          <i
            v-show="!showDropDir"
            @click.stop="showSwitchBtn(false)"
            class="dropup cursor"
            key="dropup"
          ></i>
          <i
            v-show="nowAdcdName !== userAdnm || nowBasinName"
            @click="mapBack()"
        ...
        <ul
          class="switch_btn_box cursor"
          tabindex="1"
          v-show="showDropDir"
          @blur.stop="mouseleaveSwitch"
        >
          <li
            @mouseover="(showList1 = true), (showList2 = false)"
            tabindex="2"
            @blur.stop="(showList1 = false), (showList2 = false)"
          >
            行政图
          ...
          <li
            @mouseover="(showList2 = true), (showList1 = false)"
            tabindex="3"
            @blur.stop="(showList1 = false), (showList2 = false)"
          >
            流域图
...
import popupsList from "./popups/popupsRegister";
  ...
  mounted() {
    ...
    realTimeTyphoon().then((res) => {
      if (res.success && res.data.length !== 0) {
        ...
        this.tfname = `${tfid.substring(0, 3)}年第${tfid.substring(
          4,
          5
        )}号${strong}${name}(${enname})`;
  ...
  methods: {
      ...
      this.map.on("singleclick", (evt) => {
        ...
        if (
          ...
          if (feature.values_.data) {
            ...
            // 调整弹窗位置
            if (
              this.pointData.mapType == "irrigationPopup" ||
              this.pointData.mapType == "ecologyPopup" ||
              this.pointData.mapType == "powerGePopup"
            ) {
              topNumber = 0.005;
            } else if (
    ...
    _drawXuOverlays(pointData) {
      ...
      for (var i = 0; i < pointData.length; i++) {
        ...
        jquery("#overly").after(
          `<div id='overly${i}' >
            <div class='adcdOverlay' mapType='${pointData[i].mapType}' lng='${
            pointData[i].lng
          }' adcd='${pointData[i].ADCD}' adnm='${pointData[i].ADNM}' lat='${
            pointData[i].lat
          }'>
              <div class="adcd_top">
                ${pointData[i].name} 
              ...
              <div class="adcd_bottom">
                ${pointData[i].value ? pointData[i].value : 0}<span>${
            pointData[i].unit
          }</span>
              </div>
            </div>
        ...
        document
          ...
          .addEventListener("click", (event) => {
            this._drawXuShuiOverlays(
              event.target.parentNode.getAttribute("mapType"),
              Number(event.target.parentNode.getAttribute("lng")),
              Number(event.target.parentNode.getAttribute("lat")),
              event.target.parentNode.getAttribute("adnm"),
              event.target.parentNode.getAttribute("adcd")
            );
          });
      }
    },
      ...
      this.jhPointLayer = new VectorLayer({
        ...
        style: (feature) => {
          ...
          if (size > 1) {
            ...
            let src = "";
            let mapZoom = this.map.getView().getZoom();
            if (mapZoom >= 11) {
