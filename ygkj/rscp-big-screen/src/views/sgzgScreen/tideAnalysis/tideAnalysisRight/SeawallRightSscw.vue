        ...
        <el-table :height="cwztableHeight" v-loading="cwzfbTableLoading" :data="cwzfbTableData" stripe style="width: 100%" @row-click="clickCwzfb" :span-method="objectSpanMethod">
          ...
          <el-table-column label="实时潮位" align="center">
            ...
            <el-table-column label="潮位/m" align="center">
              <template slot-scope="scope">
                <div :class="scope.row.colorType">{{scope.row.tide ? scope.row.tide : '-'}}</div>
              </template>
            </el-table-column>
          </el-table-column>
          ...
          if(res.data.length){
            ...
            res.data.map((v,i) => {
              ...
              if(v.tide > v.TIDE_ALERT_BLUE){
                if(v.tide < v.TIDE_ALERT_YELLOW || v.tide == v.TIDE_ALERT_YELLOW){
                  colorType = 'blue'
                }else if(v.tide < v.TIDE_ALERT_ORANGE || v.tide == v.TIDE_ALERT_ORANGE){
                  colorType = 'yellow'
                }else if(v.tide < v.TIDE_ALERT_RED || v.tide == v.TIDE_ALERT_RED){
                  colorType = 'orange'
                }else{
                  colorType = 'red'
                }
        ...
        res.rows.map((v,i) => {
          ...
          if(v.tide > v.tideAlertBlue){
            if(v.tide < v.tideAlertYellow || v.tide == v.tideAlertYellow){
              colorType = 'blue'
            }else if(v.tide < v.tideAlertOrange || v.tide == v.tideAlertOrange){
              colorType = 'yellow'
