<!--
 * @Author: hanyu
 * @LastEditTime: 2021-12-03 10:07:42
 * @Description: 地图右侧-病险水库
 * @FilePath: /rscp-big-screen/src/views/bigScreen/reservoir/reservoirRight/ReservoirRightOne.vue
-->
<template>
  <div class="wrap">
...
export default {
  ...
  methods: {
    ...
    async getSquareData() {
      ...
      if (this.leftSelectProjectScale) {
        opt = {
          statistic: "overCnt",
          adcd: this.adcd,
          itemCategory: "firstClass,scale",
          itemValue: "一类坝," + this.leftSelectProjectScale,
          superviseLevel: this.superviseLevel,
        };
      } else {
        opt = {
    ...
    async getStatusData() {
      ...
      if (this.leftSelectProjectScale) {
        ...
        opt = {
          statistic: "problemCnt",
          adcd: this.adcd,
          itemCategory: "firstClass," + this.squareValue,
          itemValue: "一类坝,1",
          superviseLevel: this.superviseLevel,
        };
      }
    ...
    clickStatus(index) {
      ...
      if (this.listFlag) {
        if (this.statusIndex === index) {
          ...
          this.opt.itemCategory = "firstClass,firstClass," + this.squareValue;
          this.opt.itemValue = "一类坝," + this.statusValue + ",1";
        }
        this.opt.adcd = this.leftSelectAdcd
          ? this.leftSelectAdcd
          : this.$localData("get", "userInfo").adcd;
    ...
    async getSghtfbData() {
      ...
      let res = await reservoirLeftStatisticApi({
        statistic: "adcd",
        itemCategory: "firstClass," + this.squareValue + ",scale",
        itemValue: "一类坝,1," + this.scale,
        adcd: this.leftSelectAdcd
          ? this.leftSelectAdcd
          : this.$localData("get", "userInfo").adcd,
        superviseLevel: this.superviseLevel,
    ...
    async getReservoirList(colorNum) {
      if (this.listFlag) {
        ...
        if (res.success) {
          ...
          if (res.data.length) {
            ...
            res.data.map((v) => {
              ...
              if (this.squareIndex === "") {
                ...
                // ) {
                //   v.colorType = "blue";
                // } else if (v.NWL < v.waterLevel && v.waterLevel < v.DSFLZ) {
                //   v.colorType = "yellow";
                // } else if (v.waterLevel > v.DSFLZ) {
                //   v.colorType = "orange";
                // } else {
                //   v.colorType = "green";
                // }
                ...
                (v.waterLevel < v.limitWaterLevel || v.waterLevel == v.limitWaterLevel)
                  ? (v.colorType = "green")
                  : "";
                ...
                v.NWL != null &&
                v.limitWaterLevel < v.waterLevel &&
                (v.waterLevel < v.NWL || v.waterLevel == v.NWL)
                  ? (v.colorType = "blue")
                  : "";
                ...
                v.NWL != null &&
                v.NWL < v.waterLevel &&
                (v.waterLevel < v.DSFLZ || v.waterLevel == v.DSFLZ)
                  ? (v.colorType = "yellow")
                  : "";
          ...
          // this.$parent.showMapPointJh(pointArr, 'all')
        }
        this.listFlag = true;
    ...
    clickSquare(index) {
      ...
      if (this.listFlag) {
        if (this.squareIndex === index) {
          this.squareIndex = "";
          this.squareValue = "";
          this.opt.itemCategory = "firstClass,scale";
          this.opt.itemValue = "一类坝," + this.scale;
          // this.opt.itemCategory = "status";
          // this.opt.itemValue = "一类坝";
        } else {
          this.squareIndex = index;
          this.squareValue = this.squareList[index].type;
          this.opt.itemCategory = "firstClass,scale," + this.squareValue;
          this.opt.itemValue = "一类坝," + this.scale + ",1";
