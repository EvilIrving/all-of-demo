<!--
 * @Author: hanyu
 * @LastEditTime: 2022-04-22 15:00:30
 * @Description: 三类坝
 * @FilePath: /rscp-big-screen/src/views/rscpDriveScreen/reservoir/reservoirRight/ReservoirRightBxsk.vue
-->
<template>
  <div class="wrap">
    ...
    async getSquareData() {
      ...
      if (res.success) {
        this.squareList.push(
          ...
          //   type: 'DDZ',
          //   name: '超死水位1m',
          //   value: res.data[0]['DDZ'],
          // },
          {
            // type: "WTELE",// 2022-04-14 之后用这个
            // name: "堰顶溢流", // 2022-04-14 之后用这个
            // value: res.data[0]["WTELE"],// 2022-04-14 之后用这个
            type: "DSFLZ",
            name: "超设计洪水位",
          ...
          // {
          //   type: "normal",
          //   // name: "空库(限制)运行",// 2022-04-14 之后用这个
          //   name: "低于限制水位",
    ...
    async getStatusData() {
      ...
      if (this.leftSelectProjectScale) {
        ...
        opt = {
          statistic: "problemCnt",
          adcd: this.adcd,
          itemCategory: "thirdClass," + this.squareValue,
          itemValue: "三类坝,1",
          superviseLevel: this.superviseLevel,
        };
      }
    ...
    async getSghtfbData() {
      ...
      if (this.statusValue) {
        if (this.squareValue) {
          opt = {
            statistic: "adcd",
            itemCategory: "thirdClass,thirdClass," + this.squareValue + ",scale",
          ...
          opt = {
            statistic: "adcd",
            itemCategory: "thirdClass,thirdClass,scale",
        ...
        if (this.squareValue) {
          opt = {
            statistic: "adcd",
            itemCategory: "thirdClass," + this.squareValue + ",scale",
    ...
    clickSquare(index) {
      if (this.listFlag) {
        if (this.squareIndex === index) {
          ...
          this.squareValue = "";
          if (this.statusValue) {
            this.opt.itemCategory = "thirdClass,thirdClass,scale";
            this.opt.itemValue =
              "三类坝," + this.statusValue + "," + this.scale;
          } else {
            this.opt.itemCategory = "thirdClass,scale";
            this.opt.itemValue = "三类坝," + this.scale;
          }
        } else {
          this.squareIndex = index;
          this.squareValue = this.squareList[index].type;
          this.opt.itemCategory = "thirdClass,thirdClass,scale," + this.squareValue;
          this.opt.itemValue =
    ...
    clickStatus(index) {
      if (this.listFlag) {
        if (this.statusIndex === index) {
          ...
          this.opt.itemCategory = "thirdClass,thirdClass," + this.squareValue;
