<!--
 * @Author: hanyu
 * @LastEditTime: 2021-12-03 10:06:09
 * @Description: 地图右侧-病险水库
 * @FilePath: /rscp-big-screen/src/views/bigScreen/reservoir/reservoirRight/ReservoirRightTwo.vue
-->
<template>
  <div class="wrap">
    <!-- <section class="xsgs_box">
      ...
      <div class="xsgs_words" v-loading="loading_xsgs">
        截至今日病险水库<span class="red">{{topData.total}}</span>座。
        <p>
          <span class="weight">控制运行：</span>
          空库运行{{squareList.length ? squareList[0].value : 0}}座；
          超限制水位<span class="red">{{squareList.length ? squareList[1].value : 0}}</span>座，
          超正常水位<span class="red">{{squareList.length ? squareList[2].value : 0}}</span>座。
        </p>
        <p>
          <span class="weight">加固处理：</span>
          未加固{{topData.status1}}，加固中{{topData.status2}}座，已完工{{topData.status3}}座，拟降等报废{{topData.status4}}座。
        </p>
      </div>
    </section> -->
    ...
    async getSquareData() {
      ...
      if (this.leftSelectProjectScale) {
        opt = {
          statistic: "overCnt",
          adcd: this.adcd,
          itemCategory: "secondClass,scale",
          itemValue: "二类坝," + this.leftSelectProjectScale,
          superviseLevel: this.superviseLevel,
        };
      } else {
        opt = {
    ...
    async getStatusData() {
      ...
      if (this.leftSelectProjectScale) {
        ...
        opt = {
          statistic: "problemCnt",
          adcd: this.adcd,
          itemCategory: "secondClass," + this.squareValue,
          itemValue: "二类坝,1",
          superviseLevel: this.superviseLevel,
        };
      }
    ...
    async getSghtfbData() {
      ...
      if (this.statusValue) {
        if (this.squareValue) {
          opt = {
            statistic: "adcd",
            itemCategory: "secondClass,secondClass," + this.squareValue + ",scale",
            itemValue: "二类坝," + this.statusValue + ",1," + this.scale,
            adcd: this.leftSelectAdcd
              ? this.leftSelectAdcd
              : this.$localData("get", "userInfo").adcd,
            superviseLevel: this.superviseLevel,
          ...
          opt = {
            statistic: "adcd",
            itemCategory: "secondClass,secondClass,scale",
            itemValue: "二类坝," + this.statusValue + "," + this.scale,
            adcd: this.leftSelectAdcd
              ? this.leftSelectAdcd
              : this.$localData("get", "userInfo").adcd,
            superviseLevel: this.superviseLevel,
        ...
        if (this.squareValue) {
          opt = {
            statistic: "adcd",
            itemCategory: "secondClass," + this.squareValue + ",scale",
            itemValue: "二类坝,1," + this.scale,
            adcd: this.leftSelectAdcd
              ? this.leftSelectAdcd
              : this.$localData("get", "userInfo").adcd,
            superviseLevel: this.superviseLevel,
    ...
    clickSquare(index) {
      if (this.listFlag) {
        if (this.squareIndex === index) {
          ...
          this.squareValue = "";
          if (this.statusValue) {
            this.opt.itemCategory = "secondClass,secondClass,scale";
            this.opt.itemValue =
              "二类坝," + this.statusValue + "," + this.scale;
          } else {
            this.opt.itemCategory = "secondClass,scale";
            this.opt.itemValue = "二类坝," + this.scale;
          }
        } else {
          this.squareIndex = index;
          this.squareValue = this.squareList[index].type;
          this.opt.itemCategory = "secondClass,secondClass,scale," + this.squareValue;
          this.opt.itemValue =
            "二类坝," + this.statusValue + "," + this.scale + ",1";
        }
        this.opt.adcd = this.leftSelectAdcd
          ? this.leftSelectAdcd
          : this.$localData("get", "userInfo").adcd;
    ...
    clickStatus(index) {
      if (this.listFlag) {
        if (this.statusIndex === index) {
          ...
          this.opt.itemCategory = "secondClass,secondClass," + this.squareValue;
          this.opt.itemValue = "二类坝," + this.statusValue + ",1";
        }
        this.opt.adcd = this.leftSelectAdcd
          ? this.leftSelectAdcd
          : this.$localData("get", "userInfo").adcd;
