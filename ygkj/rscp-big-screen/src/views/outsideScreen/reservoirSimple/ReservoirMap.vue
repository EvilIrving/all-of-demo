<!--
 * @Author: hanyu
 * @LastEditTime: 2022-08-05 18:32:56
 * @Description: 
 * @FilePath: \rscp-big-screen\src\views\outsideScreen\reservoirSimple\ReservoirMap.vue
-->
<template>
  <div class="map_wrap">
    ...
    <div class="popup_map" ref="popup" v-show="currentCoordinate">
      <!-- 工程气泡 有照片视频 -->
      <div class="proj_content" v-show="popupData.rightType == '1' && !popupData.hoverType">
          ...
          <div class="proj_left">
            ...
            <div class="proj_detail">
              ...
              <div class="popup_item">
                <p>所在市：</p>
                <span>{{
                  popupData.cityADNM ? popupData.cityADNM : "-"
                }}</span>
              </div>
              <div class="popup_item">
                <p>所在县：</p>
              ...
              <div class="popup_item">
                <p>安全鉴定：</p>
                <span v-if="popupData.status && popupData.status.slice(0,3) == '三类坝'">{{ popupData.status.slice(0,3) }}</span>
                <span v-else>{{ popupData.status ? popupData.status : "-" }}</span>
              </div>
              <div class="popup_item" v-if="popupData.status && popupData.status.slice(0,3) == '三类坝'">
                <p>除险加固：</p>
                <span>{{ popupData.status ? popupData.status.slice(3) : "-" }}</span>
              </div>
            </div>
          </div>
          <!-- 上部分-右侧 -->
...
export default {
  ...
  methods: {
    ...
    mapInit() {
      ...
      this.map.on("pointermove", (e) => {
        ...
        if (hit) {
          ...
          const hdms = toStringHDMS(toLonLat([popupDataHover.lng, popupDataHover.lat])); // 转换坐标格式
          this.currentCoordinateHover = hdms; // 保存坐标点
          setTimeout(() => {
            this.hoverOverlay.setPosition([popupDataHover.lng, popupDataHover.lat]);
          });
        } else {
      ...
      this.map.on("singleclick", (evt) => {
        ...
        if (
          ...
          if (feature.values_.features.length == 1) {
            ...
            setTimeout(()=>{
              this._setView(Number(pointData.LGTD) + 0.004, Number(pointData.LTTD) + 0.013);
            },300)
    ...
    async getAdcdPointsData(adnm){
      this.map.removeLayer(this.adcdPolygonLayer);
      console.log(adnm,'adnm');
      if(adnm == '浙江省'){
        return
      }
      let that = this
      let url = 'https://sldtpt.zjwater.com:6443/zjsslgcyxglpt/arcgis/rest/services/BOU/BOU_City/MapServer/0/query'
      jquery.ajax({
        url: url,
        type: 'get',
        dataType: 'jsonp',
...
function callbackAdcd(res){
  console.log(res,'callbackAdcd');
}
</script>
