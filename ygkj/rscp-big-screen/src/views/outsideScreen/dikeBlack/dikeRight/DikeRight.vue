<!--
 * @Author: hanyu
 * @LastEditTime: 2022-03-28 16:14:10
 * @Description: 流域大屏右侧-堤防
 * @FilePath: /rscp-big-screen/src/views/outsideScreen/dikeBlack/dikeRight/DikeRight.vue
-->
<template>
  <div class="wrap" ref="blackRightWrap">
    <section class="scale_box" ref="scaleBox">
      <div class="chart_title_black">
        工程规模
        <div class="chart_clean" v-if="scale != 'all'" @click="cleanScaleClick">清除</div>
      </div>
      <basin-single-bar-chart ref="scaleChart" v-loading="scaleLoading" v-if="showScaleChart" class="scaleChart" :chartData="scaleData" @barClick="scaleChartClick"></basin-single-bar-chart>
    </section>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in tableData" :key="index" @click="clickReservoir(item)">
              ...
              <td style="width:25%">{{item.project_scale}}级</td>
...
export default {
  ...
  methods: {
    ...
    async getStatusData() {
      ...
      this.$nextTick(()=>{
        this.tableHeight = (this.$refs.blackRightWrap.clientHeight - this.$refs.scaleBox.clientHeight - this.$refs.cxjgBox.clientHeight) + 'px'
      })
    },
