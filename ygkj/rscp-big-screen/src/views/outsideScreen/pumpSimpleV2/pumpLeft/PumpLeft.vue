<!--
 * @Author: hanyu
 * @LastEditTime: 2022-07-11 16:27:07
 * @Description: 地图左侧-统计
 * @FilePath: \rscp-big-screen\src\views\outsideScreen\pumpSimpleV2\pumpLeft\PumpLeft.vue
-->
<template>
  <div class="map_left">
...
export default {
  ...
  mounted() {
    this.barHeight = this.$parent.$refs.drawLeft.clientHeight / 2 + "px";
    this.getBasinData();
  ...
  methods: {
        ...
        if(res.data.length){
          for(let i = 0; i < res.data.length; i++){
            ...
            for(let j = 0; j < res.data[i].list.length; j++){
              if(res.data[i].list[j].projectCategory == '泵站'){
    ...
    //   let res = await pumpStatisticApi({
    //     adcd: this.leftSelectAdcd ? this.leftSelectAdcd : this.$localData("get", "userInfo").adcd,
    //     statistic: "power",
    //     basin: this.basinCode,
    //     superviseLevel: this.superviseLevel
    ...
    //         // yData1.push((res.data[i].INSTALLED_FLOW / 100).toFixed(2))
    //         yData1.push(res.data[i].INSTALLED_FLOW)
    //         yData2.push((res.data[i].MOTOR_POWER / 1000).toFixed(2))
