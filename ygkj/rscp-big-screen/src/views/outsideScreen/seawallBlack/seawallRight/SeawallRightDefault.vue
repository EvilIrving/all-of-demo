<!--
 * @Author: hanyu
 * @LastEditTime: 2021-10-28 17:42:30
 * @Description: 地图右侧-潮位预警
 * @FilePath: /rscp-big-screen/src/views/bigScreen/seawallBlack/seawallRight/SeawallRightDefault.vue
-->
<template>
  <div class='wrap'>
    <section class="seawallList_box">
      <div class="chart_title_black">
        海塘分布
        <el-button style="export_btn" v-if="canExport"  plain size="small" @click="exportList()">导出</el-button>
      </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <td style="width:20%">设计潮位</td>
              <td style="width:20%">设计标准</td>
              <td style="width:20%">实时潮位</td>
          ...
          <tbody>
            <tr v-for="(item, index) in tableData" :key="index" @click="clickSeawall(item)">
              ...
              <td style="width:20%">{{ item.DESIGN_TIDE_LEVEL ? item.DESIGN_TIDE_LEVEL : "-" }}</td>
              <td style="width:20%">
                {{ item.DESIGN_TIDE_STANDARD ? item.DESIGN_TIDE_STANDARD : "-" }}
              </td>
              <td style="width:20%">{{item.tide ?item.tide + 'm' : '-'}}</td>
              <td style="width:20%">{{item.tm ?item.tm.slice(0, 16) : '-'}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <!-- <div class="table_list" ref="tableContainer"> 
        <el-table :height="tableHeight" v-loading="tableLoading" :data="tableData" stripe style="width: 100%" @row-click="clickSeawall">
          ...
          <el-table-column prop="NAME" width="60px" label="海塘名称"></el-table-column>
