<!--
 * @Author: hanyu
 * @LastEditTime: 2021-09-11 12:16:41
 * @Description: 地图右侧-潮位预警
 * @FilePath: /rscp-big-screen/src/views/bigScreen/seawallSimple/seawallRight/SeawallRightCwyj.vue
-->
<template>
  <div class="wrap">
    ...
    <section class="fangan_box">
      <div class="chart_title_black">
        <p>方案选择</p>
        <el-button plain size="small" @click="fanganManage()"
          >方案管理</el-button
        >
      </div>
      <div id="fangan_select" v-if="fanganList.length">
        <el-button
        width='100%'
         :class="['planItem', index === fanganIndex ? 'active' : '']"
          v-for="(item, index) in fanganList"
          :key="index"
          :type="index == fanganIndex ? 'primary' : ''"
          @click="fanganClick(index)"
          >{{ item.planName }}</el-button
        >
      </div>
      ...
      <section class="seawallList_box">
          ...
          >
            <thead>
              <tr>
                ...
                <td style="width:20%" v-if="squareIndex === 2">
                  塘顶最低高程
                </td>
                <td
                  style="width:20%"
                  v-if="squareIndex === 0 || squareIndex === 1"
                >
                  安全状态
                </td>
                <td
                  style="width:20%"
                  v-if="
                    squareIndex === '' || squareIndex === 0 || squareIndex === 1
                  "
                >
                  设计潮位
                </td>
            ...
            <tbody>
              ...
              >
                ...
                <td style="width:20%"  v-if="squareIndex === 0 || squareIndex === 1">
                ...
                >
                  {{
                    item.DESIGN_TIDE_LEVEL ? item.DESIGN_TIDE_LEVEL + 'm' : "-"
                  }}
                </td>
                <td
                  style="width:20%"
                >
                  {{ item.forecast ? item.forecast + "m" : "-" }}
                </td>
                <td style="width:20%">
                  {{
                    item.forecast_time ? item.forecast_time.slice(0, 16) : "-"
                  }}
                </td>
              </tr>
            </tbody>
        ...
        <!-- <div class="table_list" ref="tableContainer">
          ...
          >
            ...
            <el-table-column
              :width="squareIndex === 0 || squareIndex === 1 ? '60px' : ''"
              prop="NAME"
              label="海塘名称"
            >
            ...
            <el-table-column width="60px" label="潮位" align="center">
              <template slot-scope="scope">
                <div>
                  {{ scope.row.forecast ? scope.row.forecast + "m" : "-" }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
              :width="squareIndex === 2 ? '120px' : ''"
              label="时间"
              align="center"
            >
              <template slot-scope="scope">
                <div>
                  {{
                    scope.row.forecast_time
                      ? scope.row.forecast_time.slice(0, 16)
                      : "-"
                  }}
                </div>
              </template>
    ...
    <div class="cwzfblist_wrap" v-show="cwFlag == '2'">
      ...
      <div class="table_list_diy">
        <table :style="{height:cwztableHeight}"
          :data="cwzfbTableData">
          <thead>
            <tr>
        ...
        >
          ...
          <el-table-column label="预报潮位" align="center">
            <el-table-column label="时间" align="center">
              <template slot-scope="scope">
                <div>
                  {{ scope.row.forecastTime ? scope.row.forecastTime : "-" }}
                </div>
              </template>
            </el-table-column>
            <el-table-column label="潮位/m" align="center">
              <template slot-scope="scope">
                <div :class="['bold', scope.row.colorType]">
                  {{ scope.row.forecast ? scope.row.forecast : "-" }}
                </div>
              </template>
            </el-table-column>
          </el-table-column>
