<!--
 * @Author: hanyu
 * @LastEditTime: 2022-11-29 09:39:48
 * @Description: 
 * @FilePath: \rscp-big-screen\src\views\sgzgOtherScreen\sluice\SluiceScreen.vue
-->
<template>
  <div class="screen_wrap">
    ...
    <basin-map ref="mainMap" title='智慧水闸' :show="isShowHideLayer"></basin-map>
    <section class="drawer_right" :class="{ showRight: drawerShowRight }">
      <!-- <keep-alive> -->
      <sluice-right :safeStatus="safeStatus"></sluice-right>
      <!-- </keep-alive> -->

      <!-- <img
        class="right_select_open"
        v-if="!isShowHideLayer"
        :src="$config.swImgUrl + 'expand-icon.png'"
        @click="showHideLayer()"
      /> -->
      <div
        class="right_select"
...
import SluiceLeft from "./sluiceLeft/SluiceLeft";
import SluiceRight from "./sluiceRight/SluiceRight";
...
export default {
  ...
  methods: {
    ...
    async getRightSelectList() {
      ...
      let res = await sluiceStatisticApi(this.opt);
      if (res.success){
...
.screen_wrap .drawer_right .right_select{
  @include rightSelect();
...
.screen_wrap basin-map .popup_map{
  position: fixed;
  z-index: 999;
}
