 ...
 * @Description:存量清零图例
 * @LastEditors: dtb
 * @LastEditTime: 2022-12-20 20:38:51
 * @FilePath: \rscp-big-screen\src\views\intelligentModule\skzgScreenLegend\components\Clearing.vue
-->
<template>
  <div class='wrap'>
  ...
  data() {
    return {
      ...
      skStatus: [
        {
          checked: true,
          icon: this.$config.zhImgUrl + 'earlyStage.png',
          scale: '项目前期',
        },
        {
          checked: false,
          icon: this.$config.zhImgUrl + 'reinforcing.png',
          scale: '加固中',
        },
        {
          checked: false,
          icon: this.$config.zhImgUrl + 'completed.png',
          scale: '已完工',
        },
        {
          checked: false,
          icon: this.$config.zhImgUrl + 'completedAgain.png',
          scale: '已竣工',
        }
      ],
        ...
        if (res.data.length) {
          res.data.map((v) => {
            ...
            if (v.status.indexOf('未加固') !== -1 || v.status.indexOf('拟降等报废') !== -1 || v.status.indexOf('待鉴定') !== -1) {
              v.styleObj.src = this.$config.zhImgUrl + 'earlyStage.png'
              this.pointArr['项目前期'].push(v)
            } else if (v.status.indexOf('加固中') !== -1) {
              v.styleObj.src = this.$config.zhImgUrl + 'reinforcing.png'
              this.pointArr['加固中'].push(v)
            } else if (v.status.indexOf('已完工') !== -1) {
              v.styleObj.src = this.$config.zhImgUrl + 'completed.png'
              this.pointArr['已完工'].push(v)
            } else if (v.status.indexOf('已竣工') !== -1 || v.status.indexOf('一类坝') !== -1) {
              v.styleObj.src = this.$config.zhImgUrl + 'completedAgain.png'
