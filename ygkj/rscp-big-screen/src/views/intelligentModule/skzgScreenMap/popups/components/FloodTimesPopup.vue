<!--
 * @Date: 2022-04-22 16:50:21
 * @Author: 
 * @Description: 拦洪座次 弹窗
 * @LastEditors: dtb
 * @LastEditTime: 2022-10-25 10:10:42
 * @FilePath: \rscp-big-screen\src\views\skzgScreen\skzgScreenMap\popups\components\FloodTimesPopup.vue
-->
<template>
  <div class="popup_map">
    ...
    <div class="sk_table_wrapper whitestyle" ref="wrapper">
      <!-- 工程名称、工程规模、水库功能、开始时间、结束时间、 -->
      <el-table v-loading='loading' :height='tableHeight' border stripe :data="siteList" style="width: 100%">
        ...
        <el-table-column label="开始时间" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.start_time + '时'}}</span>
          </template>
        </el-table-column>
        <el-table-column label="结束时间" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.end_time + '时'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="difflow" label="拦洪量（万m³）" align="center"></el-table-column>
      </el-table>
    </div>
...
<script>
import { floodTimesApi } from "@/api/api_reservoir";
export default {
  name: 'FloodTimesPopup',
  props: {
    popupData: {
      type: Object,
      required: true
  ...
  watch: {
    popupData(val) {
      if (val.mapType == 'floodTimesPopup') {
        this.getData()
        setTimeout(() => {
          this.resizeTable();
        }, 1000)
  ...
  methods: {
    ...
    async getData() {
      ...
      let res = await floodTimesApi(opt)
      if (res.success) {
        this.loading = false
