<template>
  <div class="map_wrap">
    ...
    <div class="map_switch">
      <div class="">
        <h2 :title="nowAdcdName || nowBasinName" class="adnm_switch">{{nowAdcdName || nowBasinName}}
          ...
          <i v-show="nowAdcdName !== userAdnm || nowBasinName" @click="mapBack()" class="el-icon-close cursor" style="font-size: 16px;margin-left: 6px;"></i>
        </h2>
        <ul class="switch_btn_box cursor" tabindex="1" v-show="showDropDir" @blur.stop="mouseleaveSwitch">
          <li @mouseover="showList1 = true,showList2=false" tabindex="2" @blur.stop="showList1 = false,showList2=false">行政图
            <!-- <i class="right_arrow cursor" /> -->
            <i class="el-icon-arrow-right cursor" />
            <div class="false_box">
              <el-cascader-panel class='adcd_box' v-model="cascaderChecked" v-show="showList1" ref="cascaderXZ" :options="adcdList" :props="{ multiple: true, checkStrictly: true,expandTrigger:'click',emitPath:false }" clearable :show-all-levels="false" @change="changeCascaderXZ">
              </el-cascader-panel>
            </div>
          </li>
          <span class="lineline"></span>
          <li @mouseover="showList2 = true,showList1=false" tabindex="3" @blur.stop="showList1 = false,showList2=false">流域图
            <!-- <i class="right_arrow cursor" /> -->
            <i class="el-icon-arrow-right cursor" />
            <div class="false_basin_box">
              <el-cascader-panel class='basin_box' v-model="cascaderChecked" v-show="showList2" ref="cascaderLY" :options="basinList" :props="{ multiple: false, checkStrictly: true,expandTrigger:'click',emitPath:false }" clearable :show-all-levels="false" @change="changeCascaderLY">
              </el-cascader-panel>
            </div>
          </li>
        </ul>
    ...
    <!-- <marquee :name='tfname' :detail='tfdetail' v-if="hasTf"></marquee> -->
    <!-- <notice-bar /> -->
    <!-- <div class="marquee">
      <div class="tfinfo" style="background: #12277454;">
        <span style="color: #EFC30A;font-family: youshe;">2022年第11号超强台风轩岚诺(Hinnamnor)</span> 9月1日15时，风速65米/秒，移速0公里/小时，东经125.60°，北纬21.70°，气压910百帕，近中心最大风力17级以上
        <i class="tf_icon"></i>
      </div>
    </div> -->
    <!-- <div class="top_num" :style="{width: showClDetail ? '100%' : 'auto'}" v-show="sgzgMapType == '除险加固'">
      <h3 @click="showClDetail = true">
        <span>存量清零情况</span>
        <img v-show="!showClDetail" :src="$config.imgUrl + 'cl-arr.png'" alt="">
      </h3>
      <p v-show="showClDetail">
        病险水库总数：<span style="color:#e6a93c" @click="chooseTopCl(1)">{{bxskNum.totalNum || 0}}</span>座；
        已清零：<span style="color:#74bd63" @click="chooseTopCl(2)">{{bxskNum.totalNum > bxskNum.stockNum ? bxskNum.totalNum - bxskNum.stockNum : 0}}</span>座；
        未清零：<span style="color:#f6685a" @click="chooseTopCl(3)">{{bxskNum.stockNum || 0}}</span>座；</p>
      <div class="img_box">
        <img v-show="showClDetail" @click="showClDetail = false" :src="$config.imgUrl + 'cl-arr2.png'" alt="">
      </div>
    </div> -->
...
import { basinStatisticApi, realTimeTyphoon } from '@/api/api_basin'

//地图弹窗注册中心
import Popups from '../skzgScreenMap/popups/Popups';
import popupsList from '@/views/skzgScreen/skzgScreenMap/popups/popupsRegister';
...
export default {
  ...
  computed: {
    ...
    skzgSwitchXsl() {
      return this.$store.state.skzgSwitchXsl
    },
    sgzgMapType() {
  ...
  methods: {
      ...
      if (res.data.length) {
        ...
        for (let i = 0; i < res.data.length; i++) {
          ...
          if (flag == 1) { // 蓄水量
            ...
            res.data[i].unit = '亿m³'
            res.data[i].mapType = 'XsQuantity'
          } else { // 蓄水率
            res.data[i].value = res.data[i].storagePerc
            res.data[i].unit = '%'
            res.data[i].mapType = 'XsRate'
          }
    ...
    _drawXuOverlays(pointData) {
      ...
      for (var i = 0; i < pointData.length; i++) {
        ...
        jquery("#overly").after(
          `<div id='overly${i}' >
            <div class='adcdOverlay' mapType='${pointData[i].mapType}' lng='${pointData[i].lng}' adcd='${pointData[i].ADCD}' adnm='${pointData[i].ADNM}' lat='${pointData[i].lat}'>
              <div class="adcd_top">
                ${pointData[i].name} 
              </div>
              <div class="adcd_bottom">
        ...
        document.getElementById("overly" + i).addEventListener('click', (event) => {
          this._drawXuShuiOverlays(event.target.parentNode.getAttribute('mapType'), Number(event.target.parentNode.getAttribute('lng')), Number(event.target.parentNode.getAttribute('lat')), event.target.parentNode.getAttribute('adnm'), event.target.parentNode.getAttribute('adcd'))
        })
      }
    },

    // 打开蓄水弹窗
    _drawXuShuiOverlays(popupRef, lng, lat, adnm, adcd) {
      this.pointData = {
        mapType: popupRef,
        adcd: adcd,
        adnm: adnm,
      ...
      this.$store.commit('SET_SKZG_SWITCH_LEGEND', false)
      this.openOverly(popupRef, lng, lat)
    },
    ...
    // 改变流域
    changeCascaderLY(itemVlaue) {
      let node = this.$refs.cascaderLY.getCheckedNodes()
      let nameArray = []
      for (let i = 0; i < node.length; i++) {
        const element = node[i];
        nameArray.push(element.label)
      }
      this.cascaderCheckedName = nameArray
      this.cascaderChecked = itemVlaue
      this.nowAdcdName = ''
      this.nowBasinName = nameArray[0]
      this.changeBasin(itemVlaue)
    },
    changeBasin(val) {
      this.initView()
      this.showDropDir = false
      let basinCode = this.basinList[val].basinCode
      let basinName = this.basinList[val].label
      this.nowAdcdName = ''
      if (basinName == '全部') {
        this.nowBasinName = ''
      } else {
        this.nowBasinName = basinName
      }
      this.getBasinLineData(basinCode)

      let baseLayer = this.map.getLayers().item(1);
      this.$store.commit('SET_SKZG_SELECT_BASIN', basinCode)
      this.$store.commit('SET_SKZG_SELECT_ADCD', '')
      // 清除市行政区划边界线
      this.map.removeLayer(this.adcdPolygonLayer)
      // 显示流域图层
