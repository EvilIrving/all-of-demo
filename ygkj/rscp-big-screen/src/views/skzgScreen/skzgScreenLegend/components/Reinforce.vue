 ...
 * @LastEditTime: 2022-04-07 15:27:47
 * @FilePath: /rscp-big-screen/src/views/skzgScreen/skzgScreenLegend/components/Reinforce.vue
-->
<template>
  <div class='wrap'>
      ...
      if (res.success) {
        ...
        if (res.data.length) {
          res.data.map((v) => {
            ...
            switch (v.status) {
              case '未开工':
                v.styleObj.src = this.$config.imgUrl + 'notStart.png'
                this.pointArr['未开工'].push(v)
                break;
              case '已开工':
                v.styleObj.src = this.$config.imgUrl + 'started.png'
                this.pointArr['已开工'].push(v)
                break;
              case '当年完工':
                v.styleObj.src = this.$config.imgUrl + 'currentYear.png'
