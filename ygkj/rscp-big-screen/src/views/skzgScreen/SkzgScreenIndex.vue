<template>
  <div class="skzg_wrap">
    <div class="skzg_top">
      <!-- <video class="lightning l-left" autoplay muted loop src="https://easyv.assets.dtstack.com/data/video/8872/6340/HbSMbf7lK-1572507146733-T6jz5ZeHvy.webm"></video>
      <video class="lightning l-right" autoplay muted loop src="https://easyv.assets.dtstack.com/data/video/8872/6340/tdxv9T5RT-1572506837636-FPIgmHEB-2.webm"></video> -->
      <p class="time">{{ nowTime }}</p>
      <p class="title">智慧水库</p>
      <div class="top-right-icon">
        <img @click="lookPhotos" :src="$config.imgUrl + 'insideBlock.png'" title="" />
      </div>
    ...
    <div class="skzg_content">
      <section class="skzg_left">
        <SkzgScreenLeft ref="skzgLeft" />
      </section>
      <section class="skzg_middle">
        <SkzgScreenMap ref="skzgMap" />
        <!-- v-permission="item.permission" -->
        <SkzgScreenLegend />
        <div class="bottom_btns">
          <div class="btns_item" v-for="(item, index) in bottomBtnList" :key="index" @click="openUrl(index)">
            {{ item.name }}
            <div v-if="showSelectBox && item.selectType" class="btns_select_box">
              <div class="btns_select_item" v-for="item in btnSelectList" :key="item.name" @click="clickBtnSelect(item.url)">
                {{ item.name }}
              </div>
            </div>
          </div>
    ...
    <primary-dialog :mainTitle="sgzgMapType" :modal="modal" :visible="dialogVisible" @handleClose="handleClose" @changeFullscreen="changeFullscreen" :fullscreen='fullscreen' width="1400px" top="5vh">
      <div class="dialog-content" :class="{'fullscreen-box': fullscreen}">
        <generation ref="generation" :fullscreen='fullscreen' v-if="dialogVisible" :sgzgSelectBasin="sgzgSelectBasin" :sgzgSelectAdcd="sgzgSelectAdcd" :checkedArr="checkedArr" :sgzgMapType="sgzgMapType" :workStatus="workStatus" :xzScale="xzScale" />
      </div>
    </primary-dialog>
    ...
    <!-- <el-dialog class="report_dialog" title="" :visible.sync="showWisdomPhotoList" width="50%" top="15vh">

    </el-dialog> -->

    <!-- <carousel-photo v-if="showWisdomPhotoList" :loading="wisdomPhotoListLoading" :photoList="titlePhotosList" @closeTitlePhotosPopup="closeTitlePhotosPopup"></carousel-photo> -->
    ...
    <primary-dialog mainTitle="我的任务" :visible="showMyWorkbench" @handleClose="closeMyWorkbench" width="75%" top="10vh">
      <div class="dialog-content">
        <SkzgScreenWorkbench v-if="showMyWorkbench" />
      </div>
    </primary-dialog>
    <primary-dialog mainTitle="" @handleClose="closeBriefReport" @changeFullscreen="changeFullscreen" :fullscreen='fullscreen' :visible="showBriefReport" width="90%" top="3vh">
      <div class="dialog-content brief-report" :class="{'fullscreen-box': fullscreen}" v-loading="briefReportLoading" element-loading-text="简报生成中,请稍等...." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
...
import SkzgScreenLeft from "./SkzgScreenLeft";
import SkzgScreenRight from "./SkzgScreenRight";
import SkzgScreenMap from "./skzgScreenMap/SkzgScreenMap";
import SkzgScreenLegend from "./skzgScreenLegend/SkzgScreenLegend";
import SkzgScreenWorkbench from "./skzgScreenWorkbench/SkzgScreenWorkbench";
  ...
  data() {
    return {
      ...
      btnSelectList: [
        {
          name: "周公宅水库",
          url: "http://115.231.59.171:8092/#/sgptlogin?userCode=121",
        },
        {
          name: "九峰水库",
          url: "http://sk.vnet1000.net/project/login/auto/2?userCode=121",
        },
        {
          name: "高坪桥水库",
          url: "https://gpq.zdwpit.cn:8443/3d.html#/main3472",
        },
        {
          name: "康门水库",
          url: "https://yhsljc.eyh.cn:800/Home/stationinfopagesl?stationId=1033894&stationType=50&userCode=123&userCode=121",
        },
        {
          name: "合溪水库",
          url: "http://hxsk.zjszsk.com/unity/reservoirs/info/pages?reservoir_id=3&userCode=121",
        },
      ],
  ...
  methods: {
      ...
      if (sgzgMapType === "安全鉴定") {
        switch (checkedArr) {
          ...
          case "二类坝":
            ...
            break;
          case "近一年到期":
            this.checkedArr = ["近一年到期"];
            break;
          case "":
            this.checkedArr = ["三类坝", "二类坝", "近一年到期"];
            break;
        }
      } else {
    ...
    lookPhotos() {
      ...
      this.titlePhotosList = [
        { url: this.$config.imgUrl + "diyizhnag.jpg", name: "1" },
        { url: this.$config.imgUrl + "dierzhang.jpg", name: "2" },
      ];
    },
