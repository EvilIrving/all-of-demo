...
public class SqlUtil {
    ...
    public static <T> List<String> batchInsertSql2(JSONArray jsonArray, String tableName, int pageSize) {
        ...
        StringBuilder basicSb = new StringBuilder("INSERT INTO ");
        basicSb.append(tableName);
        basicSb.append(" (dts_cmf_id");
        Set<String> fields = first.keySet();

        for (String fieldName : fields) {
            basicSb.append(comma).append(fieldName);
        }
        ...
        do {
            StringBuilder sb = new StringBuilder(basicSb);
            toIndex = fromIndex + pageSize;
            List<Object> subList = jsonArray.subList(fromIndex, toIndex > total ? total : toIndex);
            for (Object tt : subList) {
                LinkedHashMap t = (LinkedHashMap) tt;
