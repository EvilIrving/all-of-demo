...
public class DeptController {
    ...
    @GetMapping("/archiveDept")
    @ApiOperation(value = "档案机构列表树形结构接口")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "deptIds", value = "组织ids", paramType = "query"),
    })
    public CommonResult archiveDept(@RequestParam(required = false) String deptIds){
        return CommonResult.success(deptService.archiveDept(deptIds));
    }

    @PostMapping("/findByIds")
    @ApiOperation(value = "根据ids查找数据")
    public CommonResult findByIds(String deptIds){
        if (StringUtils.isEmpty(deptIds)) {
            CommonResult.failed("组织ids为空");
        }
        return CommonResult.success(deptService.findByIds(deptIds));
    }
}
